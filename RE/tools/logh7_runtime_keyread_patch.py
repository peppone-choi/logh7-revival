...
KEY_READ_HELPER: Final[str] = "keyReadHelper"
KEY_READ_OVERWRITE_BYTES: Final[int] = 7
KEY_READ_EVENT: Final[int] = 3


@dataclass(frozen=True, slots=True)
class RuntimeKeyReadPatch:
    source: Path
    destination: Path
    ...
    def to_json(self) -> dict[str, str | int | dict[str, str | int | dict[str, str]]]:
        return {
            ...
            "hook": {
                ...
                "returnAddressHex": f"0x{self.hook.virtual_address + KEY_READ_OVERWRITE_BYTES:08x}",
            },
            "trampoline": {
            ...
            "recordFormat": {
                ...
                "events": {"3": "keyReadHelperEntry"},
                "copyPolicy": "storedImageCopyUpTo64KeyBytes",
            },
        }
...
def _write_record_fields(builder: X86Builder, record_va: int) -> None:
    ...
    builder.append(b"\x8b\x53\x04")
    builder.append(b"\x31\xc9")
    builder.append(b"\x66\x8b\x4b\x08")
    mov_abs_from_reg(builder, b"\x89\x15", record_va + 16)
...
def _build_key_read_trampoline(
    cave: RuntimeCodeCave,
    hook: RuntimePatchTarget,
    ...
    if len(builder.data) > 512:
        raise ValueError("key read trampoline code overlaps its record buffer")
    builder.append_record_data(KEYLOG_PATH, record_va, written_va, KEYLOG_RECORD_BYTES)
    if len(builder.data) > cave.length_bytes:
        raise ValueError("key read trampoline exceeds code cave capacity")
    return bytes(builder.data)


def apply_runtime_keyread_patch(source: Path, destination: Path, manifest_out: Path) -> RuntimeKeyReadPatch:
    targets = {target.name: target for target in extract_runtime_patch_targets(source)}
    hook = targets[KEY_READ_HELPER]
    cave = find_runtime_probe_code_cave(source)
    imports = extract_runtime_probe_imports(source)
    raw = bytearray(source.read_bytes())
    original = bytes(raw[hook.file_offset : hook.file_offset + KEY_READ_OVERWRITE_BYTES])
    if original.hex() != hook.original_hex[: KEY_READ_OVERWRITE_BYTES * 2]:
        raise ValueError("keyReadHelper hook bytes do not match guarded signature")
    trampoline = _build_key_read_trampoline(cave, hook, imports, original)
    hook_bytes = hook_jump(hook.virtual_address, cave.virtual_address, KEY_READ_OVERWRITE_BYTES)
