...
def write_installed_tree(
    extracted_tree: Path,
    iso_root: Path,
    destination: Path,
    manifest_out: Path,
    playable_client: Path | None,
) -> None:
    build_installed_tree(extracted_tree, iso_root, destination, manifest_out, playable_client=playable_client)
    print(f"wrote {destination}")
    print(f"wrote {manifest_out}")
...
def _add_build_installed_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    build_parser.add_argument("--playable-client", type=Path)
    ...
    try:
        match args.command:
            ...
            case "build-installed":
                write_installed_tree(
                    args.extracted_tree,
                    args.iso_root,
                    args.out,
                    args.manifest_out,
                    args.playable_client,
                )
