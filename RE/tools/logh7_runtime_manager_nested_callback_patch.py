...
from logh7_runtime_manager_dispatcher_node_patch import _write_manager_dword
...
NESTED_TARGET: Final[str] = "runtimeManagerNestedCallbackWalker"
NESTED_LOG_PATH: Final[bytes] = b"logh7_runtime_manager_nested_callback.bin\x00"
NESTED_LOG_MAGIC: Final[bytes] = b"RMW1"
NESTED_RECORD_BYTES: Final[int] = 52
NESTED_CODE_BYTES: Final[int] = 288
NESTED_OVERWRITE_BYTES: Final[int] = 5


@dataclass(frozen=True, slots=True)
class RuntimeManagerNestedCallbackPatch:
    source: Path
    destination: Path
    ...
    def to_json(self) -> dict[str, object]:
        return {
            ...
            "logPath": NESTED_LOG_PATH.rstrip(b"\x00").decode("ascii"),
            "hook": {
                ...
                "returnAddressHex": f"0x{self.hook.virtual_address + NESTED_OVERWRITE_BYTES:08x}",
            },
            "trampoline": {
            ...
            "recordFormat": {
                "magic": NESTED_LOG_MAGIC.hex(),
                "recordBytes": NESTED_RECORD_BYTES,
                "layout": (
                    "magic,event,reserved3,contextArg,flagArg,returnAddress,currentGlobal,"
                    "member18,gate14,gate2c,listCount,listHead,firstContext,firstCallback"
                ),
            },
        }


def _write_context_byte(builder: X86Builder, context_offset: int, record_va: int, record_offset: int) -> None:
    builder.append(b"\x0f\xb6\x86")
    builder.u32(context_offset)
    builder.append(b"\xa3")
    builder.u32(record_va + record_offset)


def _build_nested_trampoline(
    *,
    cave: RuntimeCodeCave,
    hook: RuntimePatchTarget,
    ...
    _write_manager_dword(builder, 0x18, record_va, 24)
    _write_context_byte(builder, 0x14, record_va, 28)
    _write_context_byte(builder, 0x2C, record_va, 32)
    _write_manager_dword(builder, 0x28, record_va, 36)
    _write_manager_dword(builder, 0x24, record_va, 40)
    ...
    if len(builder.data) > NESTED_CODE_BYTES:
        raise ValueError("nested callback trampoline code overlaps its record buffer")
    while len(builder.data) < NESTED_CODE_BYTES:
        builder.u8(0x90)
    builder.append_record_data(NESTED_LOG_PATH, record_va, written_va, NESTED_RECORD_BYTES)
    if len(builder.data) > cave.length_bytes:
        raise ValueError("nested callback trampoline exceeds code cave capacity")
    return bytes(builder.data)


def apply_runtime_manager_nested_callback_patch(
    source: Path, destination: Path, manifest_out: Path
) -> RuntimeManagerNestedCallbackPatch:
    targets = {target.name: target for target in extract_runtime_patch_targets(source)}
    hook = targets[NESTED_TARGET]
    cave = find_runtime_probe_code_cave(source)
    imports = extract_runtime_probe_imports(source)
    raw = bytearray(source.read_bytes())
    original = bytes(raw[hook.file_offset : hook.file_offset + NESTED_OVERWRITE_BYTES])
    if original.hex() != hook.original_hex[: NESTED_OVERWRITE_BYTES * 2]:
        raise ValueError(f"{hook.name} hook bytes do not match guarded signature")
    trampoline = _build_nested_trampoline(cave=cave, hook=hook, imports=imports, original=original)
    hook_bytes = hook_jump(hook.virtual_address, cave.virtual_address, NESTED_OVERWRITE_BYTES)
    ...
    patch = RuntimeManagerNestedCallbackPatch(
        source=source,
