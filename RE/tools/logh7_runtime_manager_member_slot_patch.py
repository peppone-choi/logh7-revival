...
MEMBER_SLOT_TARGET: Final[str] = "stateTriggerMemberSlot14"
MEMBER_SLOT_LOG_PATH: Final[bytes] = b"logh7_runtime_manager_member_slot.bin\x00"
MEMBER_SLOT_LOG_MAGIC: Final[bytes] = b"RME1"
MEMBER_SLOT_RECORD_BYTES: Final[int] = 52
MEMBER_SLOT_CODE_BYTES: Final[int] = 288
MEMBER_SLOT_OVERWRITE_BYTES: Final[int] = 6


@dataclass(frozen=True, slots=True)
class RuntimeManagerMemberSlotPatch:
    source: Path
    destination: Path
    ...
    def to_json(self) -> dict[str, object]:
        return {
            ...
            "logPath": MEMBER_SLOT_LOG_PATH.rstrip(b"\x00").decode("ascii"),
            "hook": {
                ...
                "returnAddressHex": f"0x{self.hook.virtual_address + MEMBER_SLOT_OVERWRITE_BYTES:08x}",
            },
            "trampoline": {
            ...
            "recordFormat": {
                "magic": MEMBER_SLOT_LOG_MAGIC.hex(),
                "recordBytes": MEMBER_SLOT_RECORD_BYTES,
                "layout": (
                    "magic,event,reserved3,thisEcX,arg1,arg2,arg3,returnAddress,"
                    "currentGlobal,member40,member44,thisVtable,slot14Target,gateAA"
                ),
            },
        }
...
def _build_member_slot_trampoline(
    *,
    cave: RuntimeCodeCave,
    hook: RuntimePatchTarget,
    ...
    record_va = cave.virtual_address + MEMBER_SLOT_CODE_BYTES
    written_va = record_va + MEMBER_SLOT_RECORD_BYTES
    path_va = written_va + 4
    builder.append(b"\x9c\x60\xfc")
    builder.append(b"\xbf")
    builder.u32(record_va)
    builder.append(b"\xb9")
    builder.u32(MEMBER_SLOT_RECORD_BYTES)
    builder.append(b"\x31\xc0\xf3\xaa")
    builder.append(b"\xc7\x05")
    builder.u32(record_va)
    builder.append(MEMBER_SLOT_LOG_MAGIC)
    ...
    _write_manager_dword(builder, 0x40, record_va, 32)
    _write_manager_dword(builder, 0x44, record_va, 36)
    _write_manager_byte(builder, 0xAA, record_va, 48)
    builder.patch_rel8(skip_manager, builder.current_va)
    builder.append(b"\xa1")
    builder.u32(record_va + 8)
    builder.append(b"\x85\xc0")
    skip_this = builder.je_rel8_placeholder()
    builder.append(b"\x8b\x00\xa3")
    builder.u32(record_va + 40)
    builder.append(b"\xa1")
    builder.u32(record_va + 40)
    builder.append(b"\x85\xc0")
    skip_vtable = builder.je_rel8_placeholder()
    builder.append(b"\x8b\x40\x14\xa3")
    builder.u32(record_va + 44)
    builder.patch_rel8(skip_vtable, builder.current_va)
    builder.patch_rel8(skip_this, builder.current_va)
    ...
    if len(builder.data) > MEMBER_SLOT_CODE_BYTES:
        raise ValueError("member slot trampoline code overlaps its record buffer")
    while len(builder.data) < MEMBER_SLOT_CODE_BYTES:
        builder.u8(0x90)
    builder.append_record_data(MEMBER_SLOT_LOG_PATH, record_va, written_va, MEMBER_SLOT_RECORD_BYTES)
    if len(builder.data) > cave.length_bytes:
        raise ValueError("member slot trampoline exceeds code cave capacity")
    return bytes(builder.data)


def apply_runtime_manager_member_slot_patch(
    source: Path, destination: Path, manifest_out: Path
) -> RuntimeManagerMemberSlotPatch:
    targets = {target.name: target for target in extract_runtime_patch_targets(source)}
    hook = targets[MEMBER_SLOT_TARGET]
    cave = find_runtime_probe_code_cave(source)
    imports = extract_runtime_probe_imports(source)
    raw = bytearray(source.read_bytes())
    original = bytes(raw[hook.file_offset : hook.file_offset + MEMBER_SLOT_OVERWRITE_BYTES])
    if original.hex() != hook.original_hex[: MEMBER_SLOT_OVERWRITE_BYTES * 2]:
        raise ValueError(f"{hook.name} hook bytes do not match guarded signature")
    trampoline = _build_member_slot_trampoline(cave=cave, hook=hook, imports=imports, original=original)
    hook_bytes = hook_jump(hook.virtual_address, cave.virtual_address, MEMBER_SLOT_OVERWRITE_BYTES)
    ...
    patch = RuntimeManagerMemberSlotPatch(
        source=source,
