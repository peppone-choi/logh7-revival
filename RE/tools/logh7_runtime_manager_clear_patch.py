...
MANAGER_CLEAR_TARGET: Final[str] = "runtimeManagerGlobalClear"
MANAGER_CLEAR_LOG_PATH: Final[bytes] = b"logh7_runtime_manager_clear.bin\x00"
MANAGER_CLEAR_LOG_MAGIC: Final[bytes] = b"RMC1"
MANAGER_CLEAR_RECORD_BYTES: Final[int] = 32
MANAGER_CLEAR_OVERWRITE_BYTES: Final[int] = 10
...
class RuntimeManagerClearPatch:
    source: Path
    destination: Path
    ...
    def to_json(self) -> dict[str, str | int | dict[str, str | int | bool]]:
        return {
            ...
            "logPath": MANAGER_CLEAR_LOG_PATH.rstrip(b"\x00").decode("ascii"),
            "hook": {
                ...
                "returnAddressHex": f"0x{self.hook.virtual_address + MANAGER_CLEAR_OVERWRITE_BYTES:08x}",
            },
            "trampoline": {
            ...
            "recordFormat": {
                "magic": MANAGER_CLEAR_LOG_MAGIC.hex(),
                "recordBytes": MANAGER_CLEAR_RECORD_BYTES,
                "layout": "magic,event,reserved3,globalBefore,returnAddress,stackArg0,stackArg1,managerBaseCandidate",
            },
        }
...
def _append_file_write(builder: X86Builder, imports: dict[str, str], record_va: int, written_va: int, path_va: int) -> None:
    ...
    builder.append(b"\x6a")
    builder.u8(MANAGER_CLEAR_RECORD_BYTES)
...
def _build_manager_clear_trampoline(
    cave: RuntimeCodeCave,
    hook: RuntimePatchTarget,
    ...
    written_va = record_va + MANAGER_CLEAR_RECORD_BYTES
    ...
    builder.u32(MANAGER_CLEAR_RECORD_BYTES)
    builder.append(b"\x31\xc0\xf3\xaa")
    builder.append(b"\xc7\x05")
    builder.u32(record_va)
    builder.append(MANAGER_CLEAR_LOG_MAGIC)
    ...
    builder.append(b"\x8b\x44\x24\x2c\xa3")
    builder.u32(record_va + 20)
    mov_abs_from_reg(builder, b"\x89\x2d", record_va + 24)
    ...
    if len(builder.data) > 512:
        raise ValueError("runtime manager clear trampoline code overlaps its record buffer")
    while len(builder.data) < 512:
        builder.u8(0x90)
    builder.append_record_data(MANAGER_CLEAR_LOG_PATH, record_va, written_va, MANAGER_CLEAR_RECORD_BYTES)
    if len(builder.data) > cave.length_bytes:
        raise ValueError("runtime manager clear trampoline exceeds code cave capacity")
    return bytes(builder.data)


def apply_runtime_manager_clear_patch(source: Path, destination: Path, manifest_out: Path) -> RuntimeManagerClearPatch:
    targets = {target.name: target for target in extract_runtime_patch_targets(source)}
    hook = targets[MANAGER_CLEAR_TARGET]
    cave = find_runtime_probe_code_cave(source)
    imports = extract_runtime_probe_imports(source)
    raw = bytearray(source.read_bytes())
    original = bytes(raw[hook.file_offset : hook.file_offset + MANAGER_CLEAR_OVERWRITE_BYTES])
    if original.hex() != hook.original_hex[: MANAGER_CLEAR_OVERWRITE_BYTES * 2]:
        raise ValueError("runtimeManagerGlobalClear hook bytes do not match guarded signature")
    trampoline = _build_manager_clear_trampoline(cave, hook, imports, original)
    hook_bytes = hook_jump(hook.virtual_address, cave.virtual_address, MANAGER_CLEAR_OVERWRITE_BYTES)
    ...
    patch = RuntimeManagerClearPatch(
        source=source,
