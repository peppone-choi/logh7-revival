...
CHILD_ENCODE: Final[str] = "childCodecEncode"
CHILD_TRACE_PATH: Final[bytes] = b"logh7_child_codec_trace.bin\x00"
...
CHILD_ENCODE_OVERWRITE_BYTES: Final[int] = 6
CHILD_ENCODE_ENTRY_EVENT: Final[int] = 4


@dataclass(frozen=True, slots=True)
class RuntimeChildEncodePatch:
    source: Path
    destination: Path
    ...
    def to_json(self) -> dict[str, str | int | dict[str, str | int | dict[str, str]]]:
        return {
            ...
            "hook": {
                ...
                "returnAddressHex": f"0x{self.hook.virtual_address + CHILD_ENCODE_OVERWRITE_BYTES:08x}",
            },
            "trampoline": {
            ...
            "recordFormat": {
                ...
                "keyBytes": CHILD_TRACE_KEY_BYTES,
                "layout": (
                    "magic,event,reserved3,caller,codec,inputPtr,inputLength,outputHolderPtr,capacityPtr,"
                    "inputCopyLength,storedKeyPtr,storedKeyLength,storedKeyCopyLength,inputBytes64,storedKeyBytes64"
                ),
                "events": {"4": "childCodecEncodeEntry"},
                "copyPolicy": "inputAndStoredKeyImageCopyUpTo64Bytes",
            },
        }
...
def _write_record_fields(builder: X86Builder, record_va: int) -> None:
    ...
    has_input = builder.jne_rel8_placeholder()
    builder.append(b"\x31\xc9")
    builder.patch_rel8(has_input, builder.current_va)
    ...
    builder.u8(CHILD_ENCODE_ENTRY_EVENT)
    ...
    builder.append(b"\x85\xf6")
    has_key = builder.jne_rel8_placeholder()
    builder.append(b"\x31\xc9")
    builder.patch_rel8(has_key, builder.current_va)
    builder.append(b"\x83\xf9")
    builder.u8(CHILD_TRACE_KEY_BYTES)
    key_len_ok = builder.jbe_rel8_placeholder()
    builder.append(b"\xb9")
    builder.u32(CHILD_TRACE_KEY_BYTES)
    builder.patch_rel8(key_len_ok, builder.current_va)
    mov_abs_from_reg(builder, b"\x89\x0d", record_va + 44)
    builder.append(b"\x85\xc9")
    no_key_copy = builder.je_rel8_placeholder()
    builder.append(b"\xbf")
    builder.u32(record_va + 112)
    builder.append(b"\xf3\xa4")
    builder.patch_rel8(no_key_copy, builder.current_va)
...
def _build_child_encode_trampoline(
    cave: RuntimeCodeCave,
    hook: RuntimePatchTarget,
    ...
    if len(builder.data) > 512:
        raise ValueError("child encode trampoline code overlaps its record buffer")
    ...
    if len(builder.data) > cave.length_bytes:
        raise ValueError("child encode trampoline exceeds code cave capacity")
    return bytes(builder.data)


def apply_runtime_child_encode_patch(source: Path, destination: Path, manifest_out: Path) -> RuntimeChildEncodePatch:
    ...
    original = bytes(raw[hook.file_offset : hook.file_offset + CHILD_ENCODE_OVERWRITE_BYTES])
    ...
    trampoline = _build_child_encode_trampoline(cave, hook, imports, original)
