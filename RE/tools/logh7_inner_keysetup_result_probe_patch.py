...
RESULT_MAGIC: Final[bytes] = b"IKR1"
RECORD_BYTES: Final[int] = 64
RECORD_CAPACITY: Final[int] = 8
BUFFER_OFFSET: Final[int] = 200
HOOK_VA: Final[int] = 0x00613205
CONTINUATION_VA: Final[int] = 0x00613210
ORIGINAL_HEX: Final[str] = "84c07407c7472000000000"
HOOK_LENGTH: Final[int] = 11


def apply_inner_keysetup_result_probe_patch(
    ...
    if original.hex() != ORIGINAL_HEX:
        raise ValueError(f"inner keysetup result bytes drift at 0x{HOOK_VA:08x}: {original.hex()}")
    ...
    if len(trampoline) > cave.length_bytes:
        raise ValueError("inner keysetup result probe exceeds code cave capacity")
...
def _patch_json(cave: object, hook_bytes: bytes, before: int, after: int) -> dict[str, JsonValue]:
    ...
    return {
        ...
        "recordFormat": {
            "magic": RESULT_MAGIC.hex(),
            "recordBytes": RECORD_BYTES,
            "layout": "magic,callIndex,resultEax,manager,context,codec,state20,selectorOffset,listRoot,managerTail,contextCursor,contextOpcode",
        },
    }
...
def _build_trampoline(base_va: int, buffer_va: int) -> bytes:
    ...
    builder.append(b"\x84\xc0")
    skip_store = _je_rel8_placeholder(builder)
    builder.append(b"\xc7\x47\x20\x00\x00\x00\x00")
    builder.patch_rel8(skip_store, builder.current_va)
    builder.jmp_rel32(CONTINUATION_VA)
    if len(builder.data) > BUFFER_OFFSET:
        raise ValueError("inner keysetup result trampoline overlaps its ring buffer")
...
def _append_record(builder: X86Builder, buffer_va: int) -> None:
    ...
    _u32(builder, b"\xc7\x07", int.from_bytes(RESULT_MAGIC, "little"))
    builder.append(b"\x8b\x44\x24\x1c\x89\x47\x08")
    builder.append(b"\x8b\x44\x24\x04\x89\x47\x0c")
    builder.append(b"\x8b\x44\x24\x10\x89\x47\x10")
    builder.append(b"\x8b\x44\x24\x00\x89\x47\x14")
    builder.append(b"\x8b\x4c\x24\x00\x8b\x41\x20\x89\x47\x18")
    builder.append(b"\x8b\x4c\x24\x04\x0f\xb7\x41\x12\x89\x47\x1c")
    builder.append(b"\x8b\x41\x14\x89\x47\x20\x8b\x41\x1c\x89\x47\x24")
    builder.append(b"\x8b\x4c\x24\x10\x8b\x41\x04\x89\x47\x28")
    builder.append(b"\x0f\xb7\x41\x08\x89\x47\x2c")
    builder.append(b"\x61\x9d")
    _patch_rel32(builder, overflow_exit, builder.current_va)


def decode_inner_keysetup_result_ring(path: Path) -> dict[str, object]:
    data = path.read_bytes()
    if len(data) < 8:
        raise ValueError("inner keysetup result ring is too small")
    counter = struct.unpack_from("<I", data, 0)[0]
    records = _decode_records(data)
    accepted = [bool(record["accepted"]) for record in records if not record.get("empty")]
    return {
        "path": str(path),
        "bytes": len(data),
        "counter": counter,
        "innerKeysetupObserved": len(accepted) > 0,
...
def _decode_record(index: int, chunk: bytes) -> dict[str, object]:
    ci, result, manager, context, codec, state20, selector, root, tail, cursor, opcode = struct.unpack_from(
        "<IIIIIIIIIII", chunk, 4
    )
    return {
        "index": index,
        "callIndex": ci,
        "resultEaxHex": f"0x{result:08x}",
        "resultAl": result & 0xFF,
        "accepted": (result & 0xFF) != 0,
        "managerHex": f"0x{manager:08x}",
        "contextHex": f"0x{context:08x}",
        "codecHex": f"0x{codec:08x}",
        "codecState20BeforeHex": f"0x{state20:08x}",
        "managerSelectorOffsetHex": f"0x{selector:04x}",
        "managerListRootHex": f"0x{root:08x}",
        "managerTailHex": f"0x{tail:08x}",
        "contextCursorHex": f"0x{cursor:08x}",
        "contextOpcodeHex": f"0x{opcode & 0xFFFF:04x}",
    }
...
def _je_rel8_placeholder(builder: X86Builder) -> int:
    builder.append(b"\x74\x00")
    return len(builder.data) - 1
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch LOGH VII inner keysetup result ring probe.")
    ...
    if args.command == "patch":
        apply_inner_keysetup_result_probe_patch(args.source, args.out, args.manifest_out)
        print(f"wrote {args.out}")
        print(f"wrote {args.manifest_out}")
        return 0
    try:
        decoded = decode_inner_keysetup_result_ring(args.ring)
