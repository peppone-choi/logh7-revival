...
class Logh7PacketTraceTests(unittest.TestCase):
    ...
    def test_reports_world_grid_init_probe_without_followup(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            temp_path = Path(temp)
            trace = temp_path / "world-init-probe.jsonl"
            out = temp_path / "packet-analysis.json"
            trace.write_text(
                "\n".join(
                    json.dumps(item)
                    for item in [
                        ...
                        },
                        {
                            ...
                            "code": 19,
                            "transportCode": 19,
                            "queuedInternalCode": 3840,
                            "queuedInternalHex": "0x0f00",
                            "pairedInternalCode": 3841,
                            "pairedInternalHex": "0x0f01",
                            "messageName": "ResponseWorldInitialize",
                            "hex": "000a00136abdf6ddf8000105",
                        },
                        {
                            "event": "response",
                            "kind": "dynamic-world-init-candidate",
                            ...
                            "code": 20,
                            "transportCode": 20,
                            "queuedInternalCode": 3842,
                            "queuedInternalHex": "0x0f02",
                            "pairedInternalCode": 3843,
                            "pairedInternalHex": "0x0f03",
                            "messageName": "ResponseGridInitialize",
                            "hex": "000a00146abdf6ddf8000105",
                        },
                        {"event": "socket-error", "connectionId": 1, "code": "ECONNRESET"},
                        {"event": "close", "connectionId": 1},
                    ]
                )
                + "\n",
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            analysis = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(analysis["summary"]["worldInitResponseCandidates"], 2)
            self.assertEqual(analysis["summary"]["postWorldInitClientPackets"], 0)
            self.assertEqual(
                analysis["probeFindings"]["worldInitCandidateRuntimeProbe"],
                "no client packet after world/grid init candidate",
            )
            self.assertEqual(analysis["packets"][1]["frame"]["kind"], "world-grid-init-response-candidate")
            self.assertEqual(analysis["packets"][1]["frame"]["messageName"], "ResponseWorldInitialize")
            self.assertEqual(analysis["packets"][1]["frame"]["handlerInternalHex"], "0x0f01")
            self.assertEqual(analysis["packets"][2]["frame"]["messageName"], "ResponseGridInitialize")
            self.assertEqual(analysis["packets"][2]["frame"]["handlerInternalHex"], "0x0f03")
