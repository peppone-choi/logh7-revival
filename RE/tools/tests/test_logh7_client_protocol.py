...
class Logh7ClientProtocolTests(unittest.TestCase):
    def test_indexes_client_protocol_literals_and_message_symbols(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            temp_path = Path(temp)
            exe = temp_path / "G7MTClient.exe"
            out = temp_path / "client-protocol.json"
            exe.write_bytes(
                b"\x00".join(
                    [
                        b"dummy",
                        ...
                        b"LobbyLoginOK",
                        b"LobbyLoginNG",
                        b"LobbyRequestInformationCharacterCharge",
                        b"LobbyResponseInformationCharacterCharge",
                        b"LobbyRequestInformationSession",
                        b"LobbyResponseInformationSession",
                        b"LobbyCommandExtensionCharacterCharge",
                        b"LobbyCommandDeleteCharacter",
                        b"LobbySessionLoginRequest",
                        b"LobbySessionLoginOK",
                        b"LobbySessionLoginNG",
                        b"[mpsCipherManager] exchange_key_phase3: param2 length is illegal(checksum)",
                        b"[mpsCipherManager] exchange_key_phase4: broken data",
                        b"SSLoginRequest",
                        b"SSGameLoginOK",
                        b"RequestWorldInitialize",
                        b"ResponseWorldInitialize",
                    ]
                )
            )

            result = subprocess.run(
                [sys.executable, str(TOOL), "client-protocol-index", str(exe), "--out", str(out)],
                cwd=REPO_ROOT,
                text=True,
            ...
            self.assertEqual(index["defaults"]["account"], "ginei00")
            self.assertEqual(index["defaults"]["loginServerPort"], 47900)
            self.assertEqual(index["defaults"]["loginServerAddress"], "202.8.80.179")
            self.assertEqual(index["commandLineModes"][0]["mode"], "robot")
            self.assertIn("login", index["messageGroups"])
            self.assertIn("session", index["messageGroups"])
            self.assertIn("world", index["messageGroups"])
            self.assertIn("LobbyLoginRequest", index["messageGroups"]["login"])
            self.assertIn("SSLoginRequest", index["messageGroups"]["session"])
            self.assertIn("RequestWorldInitialize", index["messageGroups"]["world"])
            self.assertEqual(
                index["messageCodes"]["login"][:3],
                [
                    {"code": 8192, "hex": "0x2000", "name": "LobbyLoginRequest"},
                    {"code": 8193, "hex": "0x2001", "name": "LobbyLoginOK"},
                    {"code": 8194, "hex": "0x2002", "name": "LobbyLoginNG"},
                ],
            )
            self.assertEqual(
                index["cipherHandshake"]["phases"],
                [
                    {
                        "phase": "exchange_key_phase3",
                        "diagnostics": ["param2 length is illegal(checksum)"],
                    },
                    {"phase": "exchange_key_phase4", "diagnostics": ["broken data"]},
                ],
            )
            self.assertEqual(
                index["cipherHandshake"]["transportCodes"],
                [
                    {
                        "code": 52,
                        "hex": "0x0034",
            ...
            self.assertEqual(
                index["cipherHandshake"]["transportDispatch"],
                [
                    {
                        "transportCode": 48,
                        "transportHex": "0x0030",
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase3Param2Observations"],
                [
                    {
                        "bodyLengths": [0],
            ...
            self.assertEqual(
                index["cipherHandshake"]["adjacentTransportProbe"],
                {
                    "codes": ["0x0034", "0x0036"],
                    "bodyLengths": [0, 8, 32],
            ...
            self.assertEqual(
                index["cipherHandshake"]["requestDerivedPhase3Probe"],
                [
                    {
                        "body": "request_body24",
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase3DecodedPayloadLayout"],
                [
                    {
                        "offset": 0,
                        "field": "checksum",
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase3ChecksumAlgorithm"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase3WireEncoding"],
                {
                    "status": "unresolved",
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase3DecodedPayloadRawWireProbe"],
                [
                    {
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase1DecodedPayloadLayout"],
                [
                    {
                        "offset": 0,
                        "field": "checksum",
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase2DecodedPayloadLayout"],
                [
                    {
                        "offset": 0,
                        "field": "checksum",
            ...
            self.assertEqual(
                index["cipherHandshake"]["decodedPayloadImplementation"],
                {
                    "helpers": [
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase3ProcessingPipeline"],
                [
                    {
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase3ChildCodecVtable"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase3ChildCodecBlockTransform"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["childCodecStaticTableImplementation"],
                {
                    "helpers": [
                    ...
                    "sBoxes": 4,
                    "sBoxEntriesPerBox": 256,
                    "blockCodecStatus": "static tables, key schedule, block encode/decode, phase3 encrypted frame builder, and live phase1 replay implemented",
                    "evidence": "g012-live-replay-after-codec-fix.json",
                },
            )
            self.assertEqual(
                index["cipherHandshake"]["phase3ConfiguredResponseSchema"],
                {
                    "manifestPath": "server.gameplay.loginResponse",
                    "requestCode": 52,
                    "responseFrameCode": 53,
            ...
            self.assertEqual(
                index["cipherHandshake"]["phase3ChildCodecKeyFlow"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["requestBodyKeyAnalysis"],
                {
                    ...
                    "rejectionReason": "child-codec decode with each static/runtime candidate fails phase1/phase2/phase3 decoded-payload checksums",
                    "postCallOutputStatus": "0x6452cc post-call trace captures the exact encoded 0x0034 body before the caller frees or reuses the output holder",
                    "pythonReplayStatus": "corrected block round order reproduces the live 0x0034 body from captured plaintext and active transport key",
                    "nextTracePoint": "classify and respond to post-handshake 0x0036 and 0x0030 client packets",
                    "evidence": "g012-live-replay-after-codec-fix.json",
                },
            )
            self.assertEqual(
                index["cipherHandshake"]["phase1DerivedResponseProbe"],
                {
                    "method": "phase1-derived-guid response built from live 0x0034 request",
                    "responseMode": "phase1-derived-guid",
                    "requestFrameHex": "001a00345f01cbef3174ecd32d76704509ef162c268ea3b677430ca6",
                    "responseFrameHex": "002200357bdb991d9bdb890d2d76704509ef162c792503075087dea4d3c14951975330d7",
                    "observedFollowupFrames": [
                        ...
                        },
                        {
                            "code": 48,
                            "hex": "0x0030",
                            "frameHex": "00320030590ca783b7cecfa3797058413770ac8d752dd02709b1ee545a3107fcabf0a2d83d54a1fcb9bcd135d389f3b40cdb78ef",
            ...
            self.assertEqual(
                index["cipherHandshake"]["postHandshakeDispatchFindings"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["postHandshakeHandlerFindings"],
                {
                    ...
                    "entries": [
                        {
                            ...
                            "routeConclusion": (
                                "queued request side; no direct 0x0300 payload handler in internal dispatch"
                            ),
                        },
                        {
            ...
            self.assertEqual(
                index["cipherHandshake"]["postHandshakeFollowupAckProbe"],
                {
                    ...
                    "nextTracePoint": (
                        "decode client 0x0030 body and determine server response to internal 0x0300/0x0301"
                    ),
                },
            )
            self.assertEqual(
                index["cipherHandshake"]["postHandshakeBodyDecode"],
                {
                    ...
                    "layoutStatus": (
                        "stable decoded 48-byte body across observed runs; "
                        "field names remain semantic hypotheses"
                    ),
                    "evidence": [
            ...
            self.assertEqual(
                index["cipherHandshake"]["postHandshakeResponseCandidates"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["post0030PayloadLayout"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["post0030FollowupEffects"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["post0030FollowupEffects"]["entries"][0]["entityLookupKeyField"],
                "primaryEntry+0x00 dword",
            )
            self.assertEqual(
                index["cipherHandshake"]["post0030FollowupEffects"]["entries"][1]["motionApplyArguments"][3],
                "literal 0x3f800000 float",
            )
            self.assertEqual(
                index["cipherHandshake"]["commandOkDecodedLayouts"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["commandOkConfiguredResponseSchema"],
                {
                    "manifestPath": "server.gameplay.commandOkResponses",
                    "requestCode": 48,
                    "candidateResponseFrameCodes": [49, 50, 51],
                    "frameHex": "full LOGH transport frame, not body-only",
            ...
            self.assertEqual(
                index["cipherHandshake"]["commandOkRuntimeTraceAnalysis"],
                {
            ...
            self.assertEqual(
                index["cipherHandshake"]["dynamicGameplayProbeSchema"],
                {
                    ...
                    "requiredFields": [
                        ...
                        "transportKeyHex",
                        "decipherKeyHex",
                        "commandOkResponseCode",
                        "evidence",
                        "policy",
                    ],
                    "optionalFields": {
                        "commandOkEntityKey": "uint32 primaryEntry+0x00 dword for one-entry command OK body probes",
                    },
            ...
            self.assertEqual(
                index["cipherHandshake"]["runtimeKeyProbeFindings"],
                {
                    "debugAttach": {
                    ...
                    "memoryScan": {
                        ...
                        "result": "real login payload captured but no stored child-codec key objects found before/after login",
                    ...
                    "childCodecEncodeEntry": {
                        ...
                        "sequence": 1,
                        "transportStoredImageHex": "6c562354262420232f3a2726222e3a237422233a565255243a2653212f22202220212655246a",
                        "transportRawXor17KeyHex": "7b41344331333734382d303135392d346335342d414542332d3144363835373537363142337d",
                    ...
                    "childCodecEncodeScheduleEntry": {
                        ...
                        "transportRawXor17KeyHex": "7b41344331333734382d303135392d346335342d414542332d3144363835373537363142337d",
                        "scheduledPArrayHeadHex": "07ea4e160fbda36082588d5cd62ab3e31d393f2197ae7d8b29d1d7dcec84410e1f8a03a7d6c7b3b33c169a0f7d63d99e7fd1cfa050293d2526cc69a008db3c50",
