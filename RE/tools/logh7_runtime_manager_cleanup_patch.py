...
CLEANUP_TARGET: Final[str] = "runtimeManagerCleanupLoopEntry"
CLEANUP_LOG_PATH: Final[bytes] = b"logh7_runtime_manager_cleanup.bin\x00"
CLEANUP_LOG_MAGIC: Final[bytes] = b"CLP1"
CLEANUP_RECORD_BYTES: Final[int] = 32
CLEANUP_OVERWRITE_BYTES: Final[int] = 7
...
class RuntimeManagerCleanupPatch:
    source: Path
    destination: Path
    ...
    def to_json(self) -> dict[str, str | int | dict[str, str | int | bool]]:
        return {
            ...
            "logPath": CLEANUP_LOG_PATH.rstrip(b"\x00").decode("ascii"),
            "hook": {
                ...
                "returnAddressHex": f"0x{self.hook.virtual_address + CLEANUP_OVERWRITE_BYTES:08x}",
            },
            "trampoline": {
            ...
            "recordFormat": {
                "magic": CLEANUP_LOG_MAGIC.hex(),
                "recordBytes": CLEANUP_RECORD_BYTES,
                "layout": "magic,event,reserved3,managerArg,returnAddress,currentGlobal,selfDeleteGate,listCount,listHead",
            },
        }
...
def _append_file_write(builder: X86Builder, imports: dict[str, str], record_va: int, written_va: int, path_va: int) -> None:
    ...
    builder.append(b"\x6a")
    builder.u8(CLEANUP_RECORD_BYTES)
...
def _build_cleanup_trampoline(
    ...
    written_va = record_va + CLEANUP_RECORD_BYTES
    ...
    builder.u32(CLEANUP_RECORD_BYTES)
    builder.append(b"\x31\xc0\xf3\xaa")
    builder.append(b"\xc7\x05")
    builder.u32(record_va)
    builder.append(CLEANUP_LOG_MAGIC)
    ...
    builder.append(b"\xa1")
    builder.u32(record_va + 8)
    builder.append(b"\x8b\x40\x28\xa3")
    builder.u32(record_va + 24)
    builder.append(b"\xa1")
    builder.u32(record_va + 8)
    builder.append(b"\x8b\x40\x24\xa3")
    ...
    if len(builder.data) > 512:
        raise ValueError("runtime manager cleanup trampoline code overlaps its record buffer")
    while len(builder.data) < 512:
        builder.u8(0x90)
    builder.append_record_data(CLEANUP_LOG_PATH, record_va, written_va, CLEANUP_RECORD_BYTES)
    if len(builder.data) > cave.length_bytes:
        raise ValueError("runtime manager cleanup trampoline exceeds code cave capacity")
    return bytes(builder.data)


def apply_runtime_manager_cleanup_patch(source: Path, destination: Path, manifest_out: Path) -> RuntimeManagerCleanupPatch:
    targets = {target.name: target for target in extract_runtime_patch_targets(source)}
    hook = targets[CLEANUP_TARGET]
    cave = find_runtime_probe_code_cave(source)
    imports = extract_runtime_probe_imports(source)
    raw = bytearray(source.read_bytes())
    original = bytes(raw[hook.file_offset : hook.file_offset + CLEANUP_OVERWRITE_BYTES])
    if original.hex() != hook.original_hex[: CLEANUP_OVERWRITE_BYTES * 2]:
        raise ValueError("runtimeManagerCleanupLoopEntry hook bytes do not match guarded signature")
    trampoline = _build_cleanup_trampoline(cave, hook, imports, original)
    hook_bytes = hook_jump(hook.virtual_address, cave.virtual_address, CLEANUP_OVERWRITE_BYTES)
    ...
    patch = RuntimeManagerCleanupPatch(
        source=source,
