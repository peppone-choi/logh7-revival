...
// --- 감사 2026-06-20: 메일함/주소록 캡 + 亡命 wipe (캐논 §2.4) ---------------------------------------

test('storeMail: 메일함 120 상한 — 가득 차면 배달 차단(null)', () => {
  const s = createSocialState();
  for (let i = 0; i < 120; i += 1) {
    assert.ok(s.storeMail(9, { text: `m${i}` }), `${i}번째 저장`);
  }
  assert.equal(s.storeMail(9, { text: 'overflow' }), null, '121번째는 차단');
  assert.equal(s.getInbox(9).length, 120);
});

test('addContact: 주소록 100 상한', () => {
  const s = createSocialState();
  for (let i = 0; i < 100; i += 1) assert.equal(s.addContact(7, 1000 + i), true);
  assert.equal(s.addContact(7, 9999), false, '101번째는 거부');
  assert.equal(s.getContacts(7).length, 100);
});

test('clearContacts: 亡命 시 주소록 전체 삭제', () => {
  const s = createSocialState();
  s.addContact(7, 1); s.addContact(7, 2); s.addContact(7, 3);
  assert.equal(s.clearContacts(7), 3, '삭제된 연락처 수');
  assert.equal(s.getContacts(7).length, 0);
});
