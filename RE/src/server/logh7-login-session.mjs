...
export function createLoginSession({
  ...
  const seedPlayerCharacter = (charId, unitId) => {
    ...
    const ch = (typeof contentPack?.characterById === 'function' ? contentPack.characterById(charId) : null)
      ?? lobbyCharacters.find((c) => charIdOf(c) === charId) ?? null;
    ...
    onInnerMessage(innerPayload) {
      ...
      if (innerCode === SS_REQ_GRID_INITIALIZE_CODE) {
        ...
        if (worldPlayerEnabled()) {
          ...
          const location = currentLocationFields(activeCharacterRecord(charId));
          ...
          const extraInners = [];
          const activeRecord = activeCharacterRecord(charId);
      ...
      if (innerCode === SS_REQ_MESSENGER_STAT_CODE && gridEnterEnabled()) {
        ...
        const unitId = sessionWorldUnitId();
        seedPlayerCharacter(charId, unitId); // 戦死 판정용 플레이어 사령관 시드(flagship=unitId 링크)
        const worldChar = activeCharacterRecord(charId);
        ...
        if (battleEntryProbeEnabled()) {
          ...
          gridEnterAction.deferredBattleInners = battleSteps.map((step) => step.inner);
