...
test('processBattleOps EncourageFlagship raises morale and broadcasts 0x42c', () => {
  const state = createBattleOpsState();
  state.addUnit({ id: 909, owner: 1, morale: 100 });
  const body = Buffer.alloc(0x10);
  body.writeUInt32LE(909, 0x0c);
  ...
  assert.equal(res.accept, true);
  assert.equal(state.getUnit(909).morale, 110);
...
test('processBattleOps AirBattle broadcasts 0x428', () => {
  const state = createBattleOpsState();
  state.addUnit({ id: 71, owner: 1 });
  const body = Buffer.alloc(0x98);
  body.writeUInt8(2, 0x0c);
  body.writeUInt32LE(71, 0x10);
  body.writeUInt32LE(72, 0x14);
  const res = processBattleOps({ state, connectionId: 1, innerCode: COMMAND_AIR_BATTLE_CODE, inner: rawInner(COMMAND_AIR_BATTLE_CODE, body) });
  assert.equal(res.accept, true);
