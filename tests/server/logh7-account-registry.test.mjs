...
import { createAccountRegistry, isValidAccountLabel } from '../../src/server/logh7-account-registry.mjs';
...
function tmpDb() {
  tmpCounter += 1;
  return join(tmpdir(), `logh7-acct-${process.pid}-${tmpCounter}.json`);
}
...
test('the stored hash is salted (no plaintext, per-account salt)', () => {
  ...
  try {
    ...
    const saved = JSON.parse(readFileSync(db, 'utf8'));
    const rec = saved.accounts[0];
    assert.ok(rec.salt && rec.hash);
    assert.ok(!JSON.stringify(saved).includes(REAL_LOGIN_INNER_HEX)); // no verbatim credential persisted
  } finally {
