...
test('UnitShip: exact 0x6d64 body, count u8 @0, stride 0x8c, kind+name+floats+named stats', () => {
  const inner = buildStaticInformationUnitShipInner({
    ships: [
      {
        kind: 0x2a, name: 'SS75', shieldCapacity: 20000, beamPower: 100, speed: 5600.0, f74: 12.5,
  ...
  assert.equal(body.readUInt8(s0 + 0x0c), 4, 'name_len for "SS75"');
  assert.equal(body.readUInt16LE(s0 + 0x0d), 'S'.charCodeAt(0), 'name[0] wide char');
  // named stat block @0x54: [armorFront, armorSide, armorBack, shield, shieldCapacity, beamPower, aa, crew]
  assert.equal(body.readUInt16LE(s0 + 0x54 + 0), 390, 'armorFront @0x54');
  assert.equal(body.readUInt16LE(s0 + 0x54 + 3 * 2), 70, 'shield (defense) @0x5a');
  assert.equal(body.readUInt16LE(s0 + 0x54 + 4 * 2), 20000, 'shieldCapacity @0x5c');
  assert.equal(body.readUInt16LE(s0 + 0x54 + 5 * 2), 100, 'beamPower @0x5e');
  assert.equal(body.readFloatLE(s0 + 0x6c), 5600.0, 'speed float @0x6c');
  assert.equal(body.readFloatLE(s0 + 0x74), 12.5, 'f74 float @0x74');
...
test('createInfoRecordsStaticState seeds ship classes from content/ship-stats.json with REAL pools', () => {
  ...
  assert.equal(body.readUInt16LE(4 + 0x54 + 4 * 2), 20000, 'ship[0] shieldCapacity slot');
});
