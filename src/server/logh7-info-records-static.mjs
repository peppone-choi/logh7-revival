...
export function buildStaticInformationUnitShipInner({ ships = [] } = {}) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    ...
    writeName16(body, base + 0x0c, s.name, UNIT_SHIP_NAME_MAX); // name[≤13] (u8 len + u16 chars)
    body.writeUInt16LE(clampU16(s.w44), base + 0x44);
    body.writeUInt32LE(clampU32(s.d4c), base + 0x4c);
    // named stat block @0x54..0x67 (8 u16 slots) — fill from the high-confidence ship-stats pools where
    // provided; generic `stats[]` overrides win so a caller can place any slot exactly.
    const namedStats = [
    ...
    for (let j = 0; j < 8; j += 1) {
      ...
      body.writeUInt16LE(clampU16(v), base + 0x54 + j * 2);
    }
    body.writeFloatLE(f32(s.speed), base + 0x6c); // f6c (speed / continuous stat)
    body.writeFloatLE(f32(s.f74), base + 0x74); // f74 (second continuous stat)
    // block11 @0x7c (11 u16) — tail stat block (cost/resources/etc., unlabeled positions)
    const block11 = Array.isArray(s.block11) ? s.block11 : [];
    const tailNamed = [s.cost, s.resources, s.unitCount, s.gunPower, s.missilePower];
    for (let j = 0; j < 11; j += 1) {
      const off = base + 0x7c + j * 2;
      if (off + 2 > base + UNIT_SHIP_STRIDE) break;
      const v = block11[j] != null ? block11[j] : tailNamed[j];
...
function shipStatToUnitShip(entry, kind) {
  ...
  return {
    ...
    armorFront: rawVal('armor') ?? 0, // 装甲(前) — OCR often null; sum lives in pools.maxArmor
    armorSide: 0,
    armorBack: 0,
