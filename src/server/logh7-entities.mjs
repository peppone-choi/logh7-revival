/**
 * Persistent server-side game entities — the mutable state the server owns and saves (the data the
 * lost original server held). Each factory returns a plain JSON-serializable object with sane
 * defaults so the entity store can persist/restore it (CQRS: in-memory authoritative now, async DB
 * later). Grounded in what the client expects + the LOGH domain (nations, commanders, fleets, star
 * systems, bases, and the pending orders that carry the constmsg.dat command timing).
 */
