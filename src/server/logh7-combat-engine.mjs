 ...
 */
export function computeDamage(attacker, target, kind = 'shoot') {
  const kindFactor = kind === 'attack' ? 1.5 : kind === 'fight' ? 1.25 : 1.0;
  const raw = Math.max(1, Math.round((attacker?.beamPower ?? DEFAULT_SHIP_STATS.beamPower) * kindFactor));
  // Defense mitigates incoming damage (diminishing): effective = raw * 100/(100+defense).
  const def = Math.max(0, target?.defense ?? 0);
...
export function resolveLandCombat(attacker, defender) {
  ...
  return {
    ...
    strengthAfter,
    moraleAfter,
    attackerStrengthAfter: Math.max(0, (attacker?.strength ?? 100) - counter),
    defeated,
    result: defeated ? 1 : 0, // 1 = attacker takes the ground; 0 = ongoing (defender holds this tick)
  };
}
