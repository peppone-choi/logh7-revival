...
export function runNpcTick(state, { profileByFaction = {}, defaultProfile } = {}) {
  ...
  for (const ship of state.listShips()) {
    if (ship.destroyed || ship.owner !== 0 || ship.faction === 0) {
      continue; // only NPC-held (owner 0), faction-aligned ships act autonomously
    }
    ...
    if (decision.action === 'fire') {
      ...
      if (!target) continue;
      const dmg = computeDamage({ ...ship, beamPower: Math.round(ship.beamPower * profile.damageMul) }, target, 'shoot');
      ...
      if (dmg.destroyed) state.removeShip(target.id);
