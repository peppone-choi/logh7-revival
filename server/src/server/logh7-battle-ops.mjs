...
import { clampMoraleToMax, fleetMaxMorale, MORALE_MAX } from './logh7-morale.mjs';
 ...
 *   commander:number, x:number, y:number, z:number, control:object|null, fighters:number,
 *   supplies:number }} BattleUnit
...
export function createBattleOpsState() {
  ...
  function getOrAddUnit(id, owner = 0) {
    ...
    let u = units.get(key);
    if (!u) {
      // fighters=戦闘艇 보유 수, supplies=物資(공중전 발진 게이트용). 기본 0(시드 안 하면 발진 불가 = 보수적).
      // maxMorale=艦隊最大士気 상한(3.3): 기본 MORALE_MAX, 사령관 統率 시드 시 fleetMaxMorale로 낮춰진다.
      u = { id: key, owner, repairTag: 0, supplier: 0, morale: 100, maxMorale: MORALE_MAX, commander: 0, x: 0, y: 0, z: 0, control: null, fighters: 0, supplies: 0 };
      units.set(key, u);
    }
    return u;
  }
  ...
  return {
    ...
    addUnit({ id, owner = 0, morale = 100, commander = 0, fighters = 0, supplies = 0, leadership, maxMorale } = {}) {
      const u = getOrAddUnit(id, owner);
      u.owner = owner;
      u.commander = commander;
      u.fighters = fighters; // 戦闘艇 보유 수(공중전 對艦 피해 = fighters × 공격력)
      u.supplies = supplies; // 物資(발진 정액 10 게이트)
      // 艦隊最大士気(3.3): 명시 maxMorale > 사령관 統率(fleetMaxMorale) > 기본(MORALE_MAX). 초기 사기는 상한으로 clamp.
      u.maxMorale = Number.isFinite(maxMorale)
        ? maxMorale
        : (Number.isFinite(leadership) ? fleetMaxMorale(leadership) : MORALE_MAX);
      u.morale = clampMoraleToMax(morale, u.maxMorale);
      return u;
    },
    getUnit(id) {
...
export function processBattleOps({ state, connectionId, innerCode, inner }) {
  switch (innerCode) {
    ...
    case COMMAND_ENCOURAGE_FLAGSHIP_CODE: {
      ...
      const delta = 10;
      // 3.3: 사기 상승은 艦隊最大士気(unit.maxMorale, 統率 기반) 상한으로 clamp(기존 0x7fff 비캐논 과허용 대체).
      unit.morale = clampMoraleToMax(unit.morale + delta, unit.maxMorale);
      return {
        accept: true,
    ...
    // --- AirBattle → 0x428 (戦闘艇 공중전; air-combat 규칙엔진 연결, AU-1/opcode-wiring B-1) ---
    case COMMAND_AIR_BATTLE_CODE: {
      ...
      const targetId = cmd.unitIds[1] ?? 0;
      if (attackerId && !ownsUnit(state, attackerId, connectionId)) {
        return reject('not-owner');
      }
      // ⚠️ 보수적: 邀撃/對艦/着艦 sub-action 판별자가 0x040e body 어느 필드인지 라이브 미확정
      // (opcode-wiring §B-1, LV-5). 판별자 confirm 전까지 **단일 모드(對艦 anti-ship)로만** 연결하고
      // sub-action 분기는 보류한다. 着艦은 서버 엔진 자체가 없음.
      const mode = 'anti-ship';
      const launcher = state.getUnit(attackerId);
      const target = targetId ? state.getUnit(targetId) : null;
      // 발진 게이트: 物資 ≥ 정액 10(canLaunchFighters). 미시드/부족이면 발진 실패(result 0).
      const launcherSupplies = launcher?.supplies ?? 0;
      if (!canLaunchFighters(launcherSupplies)) {
        // 발진 불가 — 기존 형태 유지(0x0428 emit)하되 result 0(스텁 1과 구분). 상태 불변.
        return {
          accept: true,
          mode,
          launched: false,
          notifies: [{ inner: buildNotifyAirBattleInner({ attackerId, targetId, result: 0 }), target: 'all' }],
        };
      }
      // 規則엔진 호출 → 피해/감속/잔여 戦闘艇. 발진 시 物資 정액 차감(상태 변이는 여기서, 캐논 p50).
      const air = computeAirCombat(launcher ?? {}, target ?? {}, { mode });
      if (launcher) {
        launcher.supplies = Math.max(0, launcherSupplies - FIGHTER_SUPPLY_COST);
        launcher.fighters = air.launcherFightersAfter; // 對艦은 손실 없음(엔진이 보존)
      }
      if (target) {
        target.fighters = air.targetFightersAfter;
      }
      // 0x0428 body(6 dword)에 엔진값 반영: result=damage(>0이면 명중), a=damage, b=slowFactor×100,
      // c=잔여 표적 戦闘艇. result는 스텁 1이 아닌 실 피해값이라 라이브 캡처(LV-5)로 의미 confirm 가능.
      const slowPct = Math.round((air.slowFactor ?? 1) * 100);
      return {
        accept: true,
        mode,
        launched: true,
        air,
        notifies: [{
          inner: buildNotifyAirBattleInner({
            attackerId,
            targetId,
            result: air.damage ?? 0,
            a: air.damage ?? 0,
            b: slowPct,
            c: air.targetFightersAfter ?? 0,
          }),
          target: 'all',
        }],
      };
