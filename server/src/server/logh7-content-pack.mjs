...
import { normalizeSystemRecord } from './logh7-inferred-content.mjs';
...
function normalizeProvenance(value) {
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    out[key] = {
      ...entry,
      authority: entry.authority != null ? String(entry.authority) : null,
      source: sourceList(entry.source),
    };
  }
  return out;
}
...
export function createContentPack(data = DEFAULT_CONTENT) {
  ...
  const normCharacters = characters.map((ch, i) => {
    ...
    return {
      ...
      nameKo: (ch.nameKo ?? ch.name_ko ?? ch.nameKr ?? ch.name_kr) != null
        ? String(ch.nameKo ?? ch.name_ko ?? ch.nameKr ?? ch.name_kr)
        : null,
      source: sourceList(ch.source ?? ch.sources),
      provenance: normalizeProvenance(ch.provenance),
      ...
      // 캐논 NPC 월드 시드(0x0323)용 보조 필드: 진영/직위/와이어계급/성별/임시 O군 face 코드.
      // 와이어 레이아웃 무영향 — 시드 측이 이 값으로 power/rank/face/spot을 채운다.
      faction: ch.faction != null ? String(ch.faction) : null,
      postJa: ch.postJa != null ? String(ch.postJa) : null,
      // 매뉴얼 문서화 인물 플래그(P1): NPC 시드의 캐논명 unmask 게이트(추측명 P0 승격 금지).
      manualDocumented: ch.manualDocumented === true,
      wireRank: Number.isInteger(ch.wireRank) ? ch.wireRank : null,
      gender: ch.gender != null ? String(ch.gender) : null,
      faceCode: Number.isInteger(ch.faceCode) ? ch.faceCode : null,
    };
  });
  ...
  const normSystems = systemsIn.map((s, i) => normalizeSystemRecord(s, i));
  ...
  return {
    ...
    units: normUnits,
    systems: normSystems,
    institutions: normInstitutions,
    rooms: normRooms,
    resourceCatalogs: data.resourceCatalogs ?? {},
