...
// 작전계획(作戦計画) 입안/검증 순수 모듈 — MakePlan(0x0900) 입안 경로에 draft→validate 게이트를 건다.
// 발령(発令)·30일 lifecycle은 별도(아래 createStrategyState operationPlans + LOGH_OPERATION_ISSUE 게이트).
import {
  createOperationPlan,
  ...
  OPERATION_PURPOSE,
  SWEEP_RANGE_LY,
} from './logh7-operation-plan.mjs';
// 작전계획 status 상수를 전략 도메인 표면으로 재노출(작전 store 레코드의 plan.status 판별용).
export { PLAN_STATUS } from './logh7-operation-plan.mjs';
// 작전 목적/소탕 사거리 상수를 도메인 표면으로 재노출(발령자/배선이 목적·400광년 판정에 쓰도록).
export { OPERATION_PURPOSE, SWEEP_RANGE_LY } from './logh7-operation-plan.mjs';

// 발령(発令/issuePlan) 게이트: 발령 전용 sub-action opcode가 라이브 미확정(strings.tsv에 CommandMakePlan만,
// 발령 판별자 부재 — docs/logh7-opcode-wiring.md §0x0900 P3)이므로 기본 OFF. opt-in 시에만 issuePlan/30일
// 만료가 활성화된다. 게이트 off면 입안(draft)·검증·큐잉까지만, 발령은 no-op(기존 동작 보존).
export const operationIssueEnabled = () => process.env.LOGH_OPERATION_ISSUE === '1';
...
// 작전계획 자동종료 기간: 발령 후 30 in-game days(매뉴얼 p39 "発令から30日で自動終了"). P2(manual canon).
// 게임클록 day 단위. 占領/防衛/掃討 결과 보너스 수치는 별도(P2 manual, 라이브 sub-action 확정 후 정산).
export const OPERATION_DURATION_DAYS = 30; // P2: manual p39
...
export function createStrategyState({ nextOutfitId = 0x1000, operationDurationDays = OPERATION_DURATION_DAYS, targetPool = null } = {}) {
  return {
    ...
    /**
     * 작전계획(作戦計画) store — 단순 plan 큐와 병존하는, status(draft/issued)·발령시점(issuedAt 게임일)을
     * 보존하는 작전 레코드 store. 30일 lifecycle(만료) 정산은 tickOperationsIfDue가 게임클록 day로 돌린다.
    ...
    /**
     * 작전계획 store에 입안/발령된 작전을 보관한다(단순 plan 큐와 병존, 기존 plans Map은 불변). status·발령
     * 시점(issuedAt 게임일)을 보존해 30일 lifecycle 정산의 기준으로 쓴다. issuedAt=null이면 만료 미적용(발령
     * 게이트 off 시 draft만 쌓이고 만료 안 됨). 같은 planId 재보관은 덮어쓴다(중복 방지).
     */
    storeOperationPlan(power, plan, { issuedAt = null } = {}) {
      const key = power >>> 0;
      if (!this.operationPlans.has(key)) this.operationPlans.set(key, []);
      const list = this.operationPlans.get(key);
      const idx = list.findIndex((e) => e.plan?.id === plan?.id);
      // outcome 누적기: 掃討 윈도우 격침 카운트(sweepKills) 등 결과 정산 입력을 30일 동안 모은다.
      const prev = idx >= 0 ? this.operationPlans.get(key)[idx]?.outcome : null;
      const entry = { plan, issuedAt, outcome: prev ?? { sweepKills: 0 } };
    ...
    /**
     * 掃討(소탕) 작전의 윈도우 중 적함 격침 1건을 해당 진영의 발령된 掃討 작전들에 누적한다(순수 store 변경).
     * 사거리(목표 성계 400광년) 판정은 호출자(배선)가 SWEEP_RANGE_LY로 사전 필터한 격침만 넘긴다고 가정.
     * 반환=이번 격침이 누적된 작전 수(테스트/내성용). 발령 전(draft)·非掃討 작전은 누적 대상 아님.
     */
    recordSweepKill(power, { count = 1 } = {}) {
      const key = power >>> 0;
      const list = this.operationPlans.get(key);
      if (!list) return 0;
      let n = 0;
      for (const entry of list) {
        if (entry.issuedAt == null) continue; // 발령된 작전만
        if (entry.plan?.purpose !== OPERATION_PURPOSE.SWEEP) continue;
        entry.outcome = entry.outcome ?? { sweepKills: 0 };
        entry.outcome.sweepKills = (entry.outcome.sweepKills | 0) + (count | 0);
        n += 1;
      }
      return n;
    },

    /** 발령된(issued + issuedAt 있음) 작전 총수(테스트/내성용). */
    operationPlanCount() {
      let n = 0;
      for (const list of this.operationPlans.values()) n += list.length;
      return n;
    },

    /**
     * 30일 lifecycle 만료 정산(순수, 게임클록 day 주입). 발령(issued)된 작전 중 issuedAt + operationDuration
     * (30 in-game days, 매뉴얼 p39 P2)이 지난 것을 자동종료(store에서 제거)하고 만료 목록을 반환한다. 발령
     * 게이트(LOGH_OPERATION_ISSUE) off면 issuedAt=null(draft만)이라 만료 대상이 없어 no-op(회귀 0). 占領/防衛/
     * 掃討 결과 보너스는 라이브 sub-action 확정 후 별도 정산 — 이번엔 만료 제거만(보너스는 스텁).
     * @param {{ gameDay?:number, durationDays?:number }} [opts]
     * @returns {Array<{ power:number, plan:object, expiredAt:number }>} 이번 호출에서 만료된 작전들
     */
    tickOperationsIfDue({ gameDay = 0, durationDays = this._operationDurationDays, outcomeFor = null } = {}) {
      const expired = [];
      for (const [power, list] of this.operationPlans.entries()) {
        const survivors = [];
        for (const entry of list) {
          // 발령 안 됐거나(issuedAt=null) 아직 기간 미경과면 유지. 발령 후 durationDays 경과면 자동종료.
          if (entry.issuedAt == null || gameDay < entry.issuedAt + durationDays) {
            survivors.push(entry);
          } else {
            // 30일 시점 결과 정산(占領/防衛/掃討). 占領·防衛의 목표 성계 점령 상태는 외부(world-state)에서
            // 와야 하므로 outcomeFor(power, plan)→{ targetTotal, controlledByActor, lostToEnemy } 콜백으로
            // 주입; 미주입이면 누적된 sweepKills만으로 평가(占領/防衛은 보너스 0=정보부재 보수적). 보너스 수치=P2.
            const outcome = {
              ...(entry.outcome ?? {}),
              ...(typeof outcomeFor === 'function' ? (outcomeFor(power, entry.plan) ?? {}) : {}),
            };
            const evaluation = evaluateOperationOutcome(entry.plan ?? {}, outcome);
            expired.push({ power, plan: entry.plan, expiredAt: gameDay, evaluation });
          }
        }
        if (survivors.length) this.operationPlans.set(power, survivors);
...
// ============================================================================
// 작전 결과정산 → 功績(achievement) 적립 배선.
// tickOperationsIfDue가 반환한 만료 작전들(expired)의 evaluation.bonusPoints를 발령 사령관의 功績에 적립한다.
// 사령관 식별=plan.commander(발령 시 operationCtx로 주입된 발령 인물 id). draft 작전은 만료 목록에 절대 오르지
// 않으므로(issuedAt=null→tick에서 만료 안 됨) 구조적으로 제외된다(draft 功績 적립 불가). 規則=P1(발령 사령관에게
// 결과 보너스 적립), 수치=P2(보너스 분수는 operations.json P2). 순수 배선: personnelState.addAchievement만 호출.
// ============================================================================

/**
 * 만료 정산 결과(expired)를 발령 사령관의 功績에 적립한다. 각 expired 항목의 plan.commander(발령 인물 id)와
 * evaluation.bonusPoints를 읽어 personnelState.addAchievement(commander, bonusPoints)로 가산한다. commander가
 * 없거나(미발령/콜백 미주입) bonusPoints≤0이면 그 항목은 건너뛴다(占領 실패=보너스 0 → 적립 0).
 * @param {Array<{ power:number, plan:object, evaluation:object }>} expired tickOperationsIfDue 반환값
 * @param {{ addAchievement:(id:number, delta:number)=>any }} personnelState 인사 도메인 상태(功績 적립 대상)
 * @returns {Array<{ commander:number, bonusPoints:number, purpose:string, applied:boolean }>} 적립 내역
 */
export function creditOperationMerit(expired = [], personnelState = null) {
  const credits = [];
  if (!Array.isArray(expired) || typeof personnelState?.addAchievement !== 'function') {
    return credits;
  }
  for (const item of expired) {
    const commander = item?.plan?.commander;
    const bonus = Math.round(Number(item?.evaluation?.bonusPoints) || 0);
    // 발령 사령관 미상(draft/미주입)이거나 보너스 없음(실패·규칙외)이면 적립 대상 아님.
    if (commander == null || !Number.isFinite(Number(commander)) || bonus <= 0) {
      continue;
    }
    const ch = personnelState.addAchievement(commander, bonus);
    credits.push({
      commander: Number(commander) >>> 0,
      bonusPoints: bonus,
      purpose: item?.evaluation?.purpose ?? null,
      applied: ch != null, // 로스터에 없는 사령관이면 false(적립 무효)
    });
  }
  return credits;
}
 ...
 */
export function processStrategy({ state, connectionId = 0, innerCode, inner, power = 0, operationCtx = null }) {
  switch (innerCode) {
    case COMMAND_MAKE_PLAN_CODE: {
      ...
      // 목록을 주면 그것을, 없으면 planId 1개를 placeholder로(유닛 1+개 캐논 충족).
      const units = Array.isArray(operationCtx?.units) ? operationCtx.units : [parsed.planId];
      const draft = createOperationPlan({ id: parsed.planId, target: parsed.target, units });
      // 작전 목적(占領/防衛/掃討)·기본 보너스를 작전 레코드에 부착(결과 정산 입력). operationCtx 미주입(레거시
      // 경로)이면 미설정 → 정산 시 보너스 0(규칙 외)로 보수 처리, 기존 동작 불변(회귀 0).
      if (operationCtx?.purpose != null) draft.purpose = operationCtx.purpose;
      if (Number.isFinite(operationCtx?.baseBonus)) draft.baseBonus = operationCtx.baseBonus;
      // 발령 사령관(発令인물 id) 부착 — 결과정산 시 이 인물의 功績에 보너스가 적립된다(creditOperationMerit).
      // 미주입(레거시)이면 미설정 → 정산 시 적립 대상에서 제외(commander==null), 기존 동작 불변(회귀 0).
      if (Number.isFinite(operationCtx?.commander)) draft.commander = operationCtx.commander;
      const { valid, errors } = validateOperationPlan(draft, {
        validTargets: operationCtx?.validTargets ?? null,
        maxUnits: operationCtx?.maxUnits ?? Infinity,
      });
      if (!valid) {
        return { accept: false, reject: 'invalid-operation-plan', errors, notifies: [] };
      }
      // 발령 게이트(LOGH_OPERATION_ISSUE) opt-in 시: draft를 issued로 승격해 발령 게임일(issuedAt)을 박아
      // store에 보관한다(30일 lifecycle 기준). 발령 sub-action opcode 라이브 미확정이므로 게이트 off면 store에
      // draft만(issuedAt=null) 남기고 발령은 no-op → 만료 대상이 안 됨(회귀 0).
      let storedPlan = draft;
      let issuedAt = null;
      if (operationIssueEnabled()) {
        ...
        if (r.issued) {
          storedPlan = r.plan;
          issuedAt = operationCtx?.gameDay ?? 0; // 발령 게임일(미주입 시 0)
        }
      }
      state.storeOperationPlan?.(power, storedPlan, { issuedAt });
