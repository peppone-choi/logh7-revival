...
// 캐논 회랑 인접 그래프(galaxy-adjacency.json) 경로. 전략 시뮬 그래프 소스로 쓴다(이젤론/페잔 관문 정합).
const CONTENT_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../content');

// 캐논 인접 그래프를 로드한다. 없거나 파싱 실패면 null → 호출부가 euclidean 폴백.
function loadCanonAdjacency() {
  try {
    return JSON.parse(readFileSync(path.join(CONTENT_DIR, 'galaxy-adjacency.json'), 'utf8'));
  } catch {
    return null; // ponytail: 파일 부재/손상 시 조용히 euclidean 폴백(buildCanonGraph가 처리)
  }
}
...
import { createStrategicSim, buildCanonGraph, shouldBroadcastTick } from './logh7-strategic-sim.mjs';
