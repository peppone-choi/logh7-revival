...
} from './codec/outfit-party-record.mjs';
...
export const CODE_REQ_INFORMATION_BASE = 0x031e; // C→S 선택 성계 base detail 요청
...
// 0x032e RequestInformationOutfitParty → 0x032f ResponseInformationOutfitParty (고정 0x8b04B).
// 함대 정보 패널 멤버리스트 소스. handleWorldInner 가 플레이어 자기 함대 사관으로 응답한다.
export const CODE_REQ_INFORMATION_OUTFIT_PARTY = REQ_INFO_OUTFIT_PARTY_CODE;
export const CODE_RESP_INFORMATION_OUTFIT_PARTY = RESP_INFO_OUTFIT_PARTY_CODE;
export { buildResponseInformationOutfitPartyInner };
...
export const STATIC_INFO_BODY_SIZES = Object.freeze({
  ...
  [CODE_RESP_INFORMATION_OUTFIT_PARTY]: RESP_INFO_OUTFIT_PARTY_BODY_BYTES, // 35588 ResponseInformationOutfitParty
});
...
const ADMISSION_DEDICATED_BUILDERS = Object.freeze({
  ...
  // 플레이어/선택 base 문맥이 없는 fallback은 고정 크기 빈 응답이다. 정상 경로는 world-session이
  ...
  // 0x032e → 0x032f: 라우팅 인식용 등록(isAdmissionRequestCode). 실제 응답은 handleWorldInner 가
  //   플레이어 자기 함대 사관으로 가로채 생성한다(0x032a 와 동일 패턴). 여기 fallback 은 플레이어
  //   컨텍스트 없이 호출될 때의 안전망(빈 멤버리스트, count=0) — 정상 경로에선 미사용.
  [CODE_REQ_INFORMATION_OUTFIT_PARTY]: () => buildResponseInformationOutfitPartyInner(), // 0x032e → 0x032f
});
 ...
 * 0x031f/0x0321은 P0 compact 레이아웃과 현재 base id만 채우고, 미확정 상세 값은 0으로 둔다.
 */
/**
 * ★황제(emperor) 재발 금지:
...
//     함대 cell 포함) → 상세 source(0x031f + 0x0321) → 0x0f03(core 마지막) → 0x0356(post-load delta).
//
...
export function buildGridInitializeSpawnInners({
  ...
  // 8) 0x0f03 GridInitialize_OK — core grid-init 응답의 마지막. 이게 gridInitialized 를 flip 해 렌더를 트리거한다.
