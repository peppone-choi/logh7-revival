...
// LOGH7-197: 신규 캐릭터 스폰 셀 미배치(cell=0) 보정 — 세력 수도 셀 해석.
// 근거: initial-deployment.json 홈 함대 배치 = 세력 수도(제국 ヴァルハラ/オーディン=2588,
//   동맹 バーラト/ハイネセン=2014). 라이브 LOGH7-197: cell=0 → commander@+0x08=0 →
//   currentRaw11178=0 → 전략맵 함대 미선택 → 0x032e 미방출.
test('getFactionCapitalCell: 제국=2588(오딘) / 동맹=2014(하이네센) / 미상=0', () => {
  assert.equal(getFactionCapitalCell(FACTION_EMPIRE), 2588, '제국 수도 = initial-deployment imperial fleet[0].cell');
  assert.equal(getFactionCapitalCell(FACTION_ALLIANCE), 2014, '동맹 수도 = initial-deployment alliance fleet[0].cell');
  assert.equal(getFactionCapitalCell(0), 0, '미상 세력은 날조 없이 0 (호출측 defaultCell 폴백)');
  assert.equal(getFactionCapitalCell(99), 0, '미등재 세력도 0');
});
