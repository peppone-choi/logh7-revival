...
// --- (c) 특수천체 블랙홀/중성자별 distinct 값 + 進入不可 ---

test('blackHoleCells marks 블랙홀 (value 90, blocked, distinct), neutronStarCells marks 중성자별 (value 91, blocked, distinct)', () => {
  const passableCells = parsePassableCells({ rowRangesByRow: { 20: [[1, 9]] } });
  const blackHoleCells = parsePassableCells({ rowRangesByRow: { 20: [[3, 3]] } });
  const neutronStarCells = parsePassableCells({ rowRangesByRow: { 20: [[6, 6]] } });
  const { objectInner, cellInner } = buildStrategicGalaxyGrid({
    systems: [], passableCells, terrain: true, blackHoleCells, neutronStarCells,
  });
  const { decoded } = decodeCellGrid(cellInner);
  assert.equal(decoded[20 * 100 + 1], TERRAIN_VALUE.SPACE, 'lane cell stays navigable space');
  assert.equal(decoded[20 * 100 + 3], TERRAIN_VALUE.BLACK_HOLE, '블랙홀 셀 = 값 90');
  assert.equal(decoded[20 * 100 + 6], TERRAIN_VALUE.NEUTRON_STAR, '중성자별 셀 = 값 91');
  // 값 슬롯이 서로·사르가소(89)와 모두 다른지 확인.
  assert.equal(TERRAIN_VALUE.BLACK_HOLE, 90);
  assert.equal(TERRAIN_VALUE.NEUTRON_STAR, 91);
  assert.notEqual(TERRAIN_VALUE.BLACK_HOLE, TERRAIN_VALUE.NEUTRON_STAR);
  assert.notEqual(TERRAIN_VALUE.BLACK_HOLE, TERRAIN_VALUE.SARGASSO);
  // 둘 다 進入不可(class blocked, byte1=2 ∉ {1,3}), 라벨 subId는 航行不能(2) 재사용.
  assert.deepEqual(objectRecord(objectInner, TERRAIN_VALUE.BLACK_HOLE), { byte0: 2, byte1: 2, byte2: 0 }, '블랙홀: byte1=2 blocked');
  assert.deepEqual(objectRecord(objectInner, TERRAIN_VALUE.NEUTRON_STAR), { byte0: 2, byte1: 2, byte2: 0 }, '중성자별: byte1=2 blocked');
});

test('special bodies are not emitted when no cells are supplied (no guessed placement)', () => {
  const passableCells = parsePassableCells({ rowRangesByRow: { 20: [[1, 9]] } });
  // blackHoleCells/neutronStarCells 미지정 → galaxy.json엔 좌표 없음(추측 금지).
  const { objectInner, cellInner } = buildStrategicGalaxyGrid({ systems: [], passableCells, terrain: true });
  const { decoded } = decodeCellGrid(cellInner);
  // 어떤 셀도 BH/NS 값을 갖지 않는다.
  assert.ok(![...decoded].includes(TERRAIN_VALUE.BLACK_HOLE), 'no black-hole cell stamped');
  assert.ok(![...decoded].includes(TERRAIN_VALUE.NEUTRON_STAR), 'no neutron-star cell stamped');
  // 빈 입력이면 오브젝트 레코드도 비어 있다(byte1=0 미초기화 슬롯).
  assert.equal(objectRecord(objectInner, TERRAIN_VALUE.BLACK_HOLE).byte1, 0, '블랙홀 오브젝트 비어 있음');
  assert.equal(objectRecord(objectInner, TERRAIN_VALUE.NEUTRON_STAR).byte1, 0, '중성자별 오브젝트 비어 있음');
});

test('galaxy.json _specialBodies records the P1 count (3 bh + 3 ns) without guessed cell placement', () => {
  const galaxy = readJson('../../content/galaxy.json');
  const sb = galaxy._specialBodies;
  assert.ok(sb, 'galaxy.json carries a _specialBodies provenance block');
  // Null_galaxy.mdx 노드명 기준: 블랙홀 3 + 중성자별 3 (model-galaxy-stars.json special_bodies).
  assert.equal(sb.blackHoleCount, 3, '블랙홀 3개 (bh_01..bh_03)');
  assert.equal(sb.neutronStarCount, 3, '중성자별 3개 (ns_01..ns_03)');
  // 셀 배치는 캐논 매핑이 없어 미확정 — 좌표를 P0로 승격하지 않았는지 확인.
  assert.match(String(sb._grade), /^P1/, '존재/개수는 P1');
  assert.match(String(sb._cellPlacement), /UNVERIFIED|P3/, '셀 배치는 미확정(추측 좌표 금지)');
  // 어떤 시스템에도 추측된 specialBody 타입이 P0로 박히지 않았는지(필드 부재) 확인.
  for (const s of galaxy.systems) {
    assert.equal(s.specialBody, undefined, `${s.system}에 추측 specialBody 타입 없음`);
  }
});
