...
test('computeDamage: 사령관 統率 변조(opts) — 공격측 가산·방어측 경감, 미주입 시 불변', () => {
  // 統率(leadership) 전투 변조는 4번째 opts로만 작동(SERVER DESIGN, 계수 P3). opts 미주입/빈객체 = 기존 동작.
  const attacker = { beamPower: 100 };
  const target = { shield: 0, maxShield: 0, armor: 1000, maxArmor: 1000, zanki: 1000, maxZanki: 1000, defense: 0 };
  const base = computeDamage(attacker, { ...target }, 'shoot').armorDamage;
  // 빈 opts는 기존과 완전히 동일(기본 밸런스 보존 = 1071 테스트 보존의 근거).
  assert.equal(computeDamage(attacker, { ...target }, 'shoot', {}).armorDamage, base);
  // 공격측 統率 100 → 피해 증가(+25% 곡선).
  const withAttackerCmd = computeDamage(attacker, { ...target }, 'shoot', { attackerCommand: 100 }).armorDamage;
  assert.ok(withAttackerCmd > base, `attacker command raises damage (${withAttackerCmd} > ${base})`);
  // 방어측 統率 100 → 피해 감소(def +25 곡선).
  const withTargetCmd = computeDamage(attacker, { ...target }, 'shoot', { targetCommand: 100 }).armorDamage;
  assert.ok(withTargetCmd < base, `target command lowers damage (${withTargetCmd} < ${base})`);
});
...
test('resolveLandCombat: 약한 공격자가 강한 방어를 못 뚫으면 반격으로 전멸→result 2(방어측 승리)', () => {
  const r = resolveLandCombat({ strength: 1, morale: 100 }, { strength: 100, morale: 100, defense: 80 });
  assert.equal(r.defeated, false, '방어측 생존');
  assert.ok(r.attackerStrengthAfter <= 0, '공격측 반격으로 전멸');
  assert.equal(r.result, 2, 'result=2 방어측 승리');
});

test('resolveLandCombat: 방어측 생존 시 공격측에 반격피해(attackerStrengthAfter < 초기)', () => {
  const r = resolveLandCombat({ strength: 200, morale: 100 }, { strength: 100, morale: 100, defense: 60 });
  if (!r.defeated) assert.ok(r.attackerStrengthAfter < 200, '방어 생존 시 공격측 손실');
});
