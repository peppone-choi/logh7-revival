...
// 0x0317 셀렉터 레버 오라클(2026-06-26, C002 mode2 라이브-prep): 클라 case 0x317은 inner 첫 dword를
// clientBase+0x35f358에 LE u32로 복사(FUN_004ba2b0 redex 확정). 셀렉터 바이트는 +0x35f35a = dword byte[2]
// = (grid>>16)&0xff (FUN_004b68f0: byte!=0 → iVar7=1, byte==0 → iVar7=2). 서버 빌더가 byte[2]를
// 정확히 grid의 16..23 비트로 배치하는지 검증한다.
test('InformationGrid: byte[2] = (grid>>16)&0xff = 0x35f35a mode selector', () => {
  for (const grid of [0x00000000, 0x00010000, 0x00ff0000, 0x12345678, 0xdeadbeef]) {
    const inner = buildInformationGridInner({ grid });
    const body = framedBody(inner, RESP_INFORMATION_GRID_CODE, RESP_INFORMATION_GRID_BYTES);
    const expectedSelector = (grid >>> 16) & 0xff;
    assert.equal(body.readUInt8(0x02), expectedSelector, `byte[2] selector for grid=0x${grid.toString(16)}`);
    assert.equal(body.readUInt32LE(0x00) >>> 0, grid >>> 0, 'full dword round-trips');
  }
});
