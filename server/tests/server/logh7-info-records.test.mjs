 ...
 * key fields at their spec'd offsets. Body fields are little-endian except parser-helper stream fields
 * that live evidence proves are network-order before the client helper swaps them. Evidence:
 * docs/logh7-proto-info-records.md and .debug-journal.md. Pure/synchronous — no live client.
 */
...
test('Card: exact 0x520a body, count u16 @0, stride 0x46, fields + command list', () => {
  const inner = buildStaticInformationCardInner({
    cards: [
      { id: 0x1234, b02: 7, b03: 9, w06: 0xabcd, w12: 0x55aa, commands: [0x11, 0x22, 0x33] },
  ...
  assert.equal(body.readUInt16LE(c0 + 0x06), 0xabcd, 'w06');
  assert.equal(body.readUInt16LE(c0 + 0x12), 0x55aa, 'w12');
...
test('Card: command list and card count are capped to 24 / 300', () => {
  ...
  assert.equal(body.readUInt8(2 + 0x14), CARD_COMMAND_MAX, 'command_count capped at 24');
});

// ---------------------------------------------------------------------------
// 0x31d ResponseStaticInformationBase
// ---------------------------------------------------------------------------
function decodeStaticBaseStream(body, index = 0) {
  let cursor = 2;
  for (let i = 0; i <= index; i += 1) {
    const recordStart = cursor;
    const id = body.readUInt32BE(cursor);
    cursor += 4;
    const grid = body.readUInt16BE(cursor);
    cursor += 2;
    const w06 = body.readUInt16BE(cursor);
    cursor += 2;
    const w08 = body.readUInt16BE(cursor);
    cursor += 2;
    const nameLen = body.readUInt8(cursor);
    cursor += 1;
    let name = '';
    ...
    cursor += 4;
    const u2c = body.readUInt32BE(cursor);
    cursor += 4;
...
test('StaticBase: exact 0x520c body, parser-helper stream, id/grid/name', () => {
  ...
  assert.equal(body.readUInt16BE(0x00), 2, 'parser-helper outer count u16be');
  assert.equal(body.readUInt16LE(0x00), 0x0200, 'old u8-at-0 shape would byte-swap to count 0x0200');

  const first = decodeStaticBaseStream(body, 0);
  assert.equal(first.recordStart, 2, 'records start immediately after the u16be count');
  assert.equal(first.id, 42, 'system id');
  assert.equal(first.grid, 7, 'grid (map cell)');
  assert.equal(first.w06, 0, 'unresolved u16 slot stays zero');
  assert.equal(first.w08, 0, 'unresolved u16 slot stays zero');
  assert.equal(first.name, 'Odin', 'name stream');
  assert.equal(first.class_, 3, 'class_');
  assert.equal(first.diameter, 12000, 'diameter float');
  assert.equal(first.u2c, 1, 'u32 orbit slot receives truncated revolution radius');
  assert.equal(first.revolutionDirection, 1, 'revolution_direction');
  assert.ok(Math.abs(first.revolutionCycle - 2.5) < 1e-6, 'revolution_cycle float');

  const second = decodeStaticBaseStream(body, 1);
  assert.equal(second.id, 43, 'base[1] id in stream');
  assert.equal(second.grid, 8, 'base[1] grid in stream');
  assert.equal(second.name, 'Heinessen', 'base[1] name in stream');
});

// 0x31d KO-name gate: with LOGH_KO_NAMES=1 the system NAME prefers the recovered Korean rendering
// (content/names/systems-ko.json via the content source); default keeps the Japanese name. Encoding is
// unchanged (UTF-16LE code units), so Hangul round-trips like kana and the wire size stays fixed.
test('StaticBase: LOGH_KO_NAMES=1 emits the Korean system name (ルンビーニ -> 룬비니), default keeps JP', () => {
  // Read the name[<=13] (u8 len + u16 LE chars) of base[0] back from the wire.
  const readName = (inner) => {
    const body = framedBody(inner, RESP_STATIC_INFORMATION_BASE_CODE, RESP_STATIC_INFORMATION_BASE_BYTES);
    return decodeStaticBaseStream(body, 0).name;
  };

  // High-confidence db-matched system from systems-ko.json (source:"db", confidence:1.0), present in
  // galaxy.json (80/80 jp keys match), so the KO map resolves it via the content source.
  const source = openContentSource({ build: true });
  const NAME_JA = 'ルンビーニ';
  const NAME_KO = '룬비니';
  assert.equal(source.getSystemKoName(NAME_JA), NAME_KO, 'KO map resolves the db-matched system');
  const base = { id: 1, grid: 5, name: NAME_JA, name_ko: source.getSystemKoName(NAME_JA) };

  const prev = process.env.LOGH_KO_NAMES;
  try {
    delete process.env.LOGH_KO_NAMES; // default: Japanese
    assert.equal(readName(buildStaticInformationBaseInner({ bases: [base] })), NAME_JA, 'default emits JP name');

    process.env.LOGH_KO_NAMES = '1'; // gate on: Korean
    assert.equal(readName(buildStaticInformationBaseInner({ bases: [base] })), NAME_KO, 'gate-on emits KO name');

    // missing KO falls back to JP even with the flag on (additive, never blanks a name)
    assert.equal(
      readName(buildStaticInformationBaseInner({ bases: [{ id: 2, name: NAME_JA }] })),
      NAME_JA,
      'no name_ko -> JP fallback',
    );
  } finally {
...
test('InformationBase: exact 0x604 body, u8 count @0, stride 0x180, pinned fields', () => {
  const inner = buildInformationBaseInner({
    bases: [
      { id: 7, b04: 2, b05: 3, u120: 0x11223344, b175: 3 },
      { id: 8, b04: 1 },
    ],
  });
  const body = framedBody(inner, RESP_INFORMATION_BASE_CODE, RESP_INFORMATION_BASE_BYTES);
  assert.equal(RESP_INFORMATION_BASE_BYTES, 0x604);
  assert.equal(body.readUInt8(0x00), 2);
  assert.equal(INFORMATION_BASE_MAX * INFORMATION_BASE_STRIDE + 4, RESP_INFORMATION_BASE_BYTES);

  const b0 = 4;
  assert.equal(body.readUInt32LE(b0 + 0x00), 7);
  assert.equal(body.readUInt8(b0 + 0x04), 2);
  assert.equal(body.readUInt8(b0 + 0x05), 3);
  assert.equal(body.readUInt32LE(b0 + 0x120), 0x11223344);
  assert.equal(body.readUInt8(b0 + 0x175), 3);

  const b1 = 4 + INFORMATION_BASE_STRIDE;
  assert.equal(body.readUInt32LE(b1 + 0x00), 8);
  assert.equal(body.readUInt8(b1 + 0x04), 1);
});

test('InformationBase: base count is capped to four records', () => {
  const inner = buildInformationBaseInner({ bases: Array.from({ length: 10 }, (_, i) => ({ id: i + 1 })) });
  const body = framedBody(inner, RESP_INFORMATION_BASE_CODE, RESP_INFORMATION_BASE_BYTES);
  assert.equal(body.readUInt8(0x00), INFORMATION_BASE_MAX);
  assert.equal(body.readUInt32LE(4 + 3 * INFORMATION_BASE_STRIDE), 4);
  assert.equal(4 + 4 * INFORMATION_BASE_STRIDE, body.length);
});
...
test('empty inputs still emit a full zero-padded record of the dispatch size', () => {
  const checks = [
    ...
    [buildInformationBaseInner(), RESP_INFORMATION_BASE_CODE, RESP_INFORMATION_BASE_BYTES],
