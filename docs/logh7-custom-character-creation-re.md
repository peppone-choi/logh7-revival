...
- 실측 sha256 `95d8ed11f521b1588ab59af6969e49c47010f4b3dbaff2de93d15e876031515d` (size 3,956,736) — **현재 설치본(라이브 QA가 구동하는 그 EXE)**
- ImageBase 0x400000, ASLR off → VA = 파일오프셋 + 0x400000 (`.text` raw off == VA off == 0x1000).
- 근거: capstone 디스어셈블(현재 EXE 원바이트) + Ghidra 12.1.2 디컴파일(`scratchpad/ghidra-out/g7mtclient-sjis.exe_decompiled.c`, 동일 코드 이미지) 교차확인.

> **로비 잠금 해제 = 0x2004 body[0](information_count)≥1** — 단, 이 잠금은 **캐릭터 패널의 안내 텍스트/카드에만** 걸리고 **메뉴 버튼(item1/item2)을 런타임 disable하지 않는다.** item1·item2가 죽어 있는 진짜 이유는 **정적 enable 배열에서 0**이기 때문이다(런타임 잠금 오버라이드 아님). 따라서 커스텀 생성(item1)은 count와 무관하게 **정적 패치로만** 살아난다.
>
> **한 줄:** item1 enable 패치 = **파일오프셋 0x11ab3e(1157950) 바이트 `00`→`01`** (FUN_0051a370 case 0x16 `IntoLobbyMain` 정적 enable 배열의 byte[1]). 0x1008 커스텀 생성 요청/응답 wire는 기존 `docs/reference/legacy-evidence/logh7-character-creation-wire.md`에서 정적 RE + 실캡처로 이미 확정(§3에 요약). item1 클릭 → 시퀀서 state 0x19 → SS접속/인증 → state 0x41 → 위저드 FUN_00594f20(생성 분기) → 0x1008 송신.
...
0x51ab3a  c6 44 24 15 00   mov byte [esp+0x15], 0   ; item1 DISABLE  ← 패치 대상
0x51ab3f  c6 44 24 16 00   mov byte [esp+0x16], 0   ; item2 disable
...
`FUN_0050cf40(0x52)` 가 인자를 pop(stdcall)하므로 저장 시점 `[esp+0x14+i]` 와 루프 소비 시점 `[esp+0x10+i]` 는 **동일 절대주소** = byte[i]. 즉 실 enable 배열 = **`{1,0,0,0,0,1,0,1}`**.

> **주의 — 기존 문서 정정:** `docs/logh7-m2-character-creation-flow.md`·`logh7-loop-state.md`의 `{1,0,1,0,1,0,0,0}` 는 **오류**다(디컴파일 스택-coalescing 오독 추정). 실측 원바이트는 `{1,0,0,0,0,1,0,1}`. → 정적 enable은 **item0·item5·item7 뿐**, item2(오리지널 추첨)·item4(세션변경)도 **정적 disable**이다.
...
| **1** | **0x19** | **DAT_02217398=0x41 → 위저드 생성 분기** | **新キャラクターの作成 (0x1008)** | ❌ **0** |
| 2 | 0x1a | `PUSH_ORIGINAL`, DAT_02217398=0x40 | オリジナルキャラクター抽選 (0x1006) | ❌ 0 |
| 3 | 0x1b | `PUSH_DELETE`, DAT_02217398=0x39 | キャラクター削除 (0x2008) | ❌ 0 |
| 4 | 0x1c | `PUSH_SESSIONMOVE` | セッションの変更 | ❌ 0 |
| 5 | 0x1d | `PUSH_CONFIG` | 環境設定 | ✅ 1 |
| 6 | 0x27 | (state 0x27) | クレジット(추정) | ❌ 0 |
| 7 | — | `FUN_0054eed0()` 즉시 호출 | ゲーム終了 | ✅ 1 |

item1(新キャラクターの作成)은 정적 disable 확정. 클릭해도 히트테스트가 disabled 위젯을 무시 → **서버 프레임 0건·UI 무전환**(라이브 관측과 정합).

### 1.3 최소 패치 (exe-patch 매니페스트 형식)

`tools/patch/exe-patch.mjs` 규칙: `offset`(10진), `originalBytes`==파일 현재바이트여야 적용, same-length. 명령 인스트럭션 5바이트를 창으로 잡아 **imm 1바이트만** 바꾼다(오프셋 오적용 방지 앵커링).

**item1 켜기 (필수):**
```json
{
  "id": "lobby-item1-enable-create-character",
  "sourceExeSha256": "95d8ed11f521b1588ab59af6969e49c47010f4b3dbaff2de93d15e876031515d",
  "addressKind": "offset",
  "offset": 1157946,
  "originalBytes": "c644241500",
  "patchedBytes": "c644241501",
  "rollbackBytes": "c644241500",
  "reason": "Enable lobby item1 新キャラクターの作成 (CommandGenerateCharacterCharge 0x1008). FUN_0051a370 case 0x16 enable-array byte[1]: mov byte[esp+0x15],0 -> ,1"
}
```
- 진짜 최소형(1바이트)을 원하면: `offset:1157950, originalBytes:"00", patchedBytes:"01", rollbackBytes:"00"` (imm 바이트 0x11ab3e 직접). 5바이트 앵커형을 권장.

**item3 삭제(0x2008)도 켜려면 (선택):**
```json
{
  "id": "lobby-item3-enable-delete-character",
  "sourceExeSha256": "95d8ed11f521b1588ab59af6969e49c47010f4b3dbaff2de93d15e876031515d",
  "addressKind": "offset", "offset": 1157956,
  "originalBytes": "c644241700", "patchedBytes": "c644241701", "rollbackBytes": "c644241700",
  "reason": "Enable lobby item3 キャラクター削除 (LobbyCommandDeleteCharacter 0x2008)"
}
```
**item2 오리지널 추첨(0x1006)도 정적 disable이므로**, 그 경로가 필요하면 동일 패턴: `offset:1157951, originalBytes:"c644241600", patchedBytes:"c644241601"`.

> ⚠️ **매니페스트 sha 정합 필수:** 현재 `logh7-exe-patch-manifest.json`의 `targetExe.sha256`는 `2848be76…`(구 pristine-loopback)인데 **설치본 실측은 `95d8ed11…`**. apply 도구는 `targetExe.sha256==파일해시` **및** `patch.sourceExeSha256==targetExe.sha256`을 강제한다. → 패치 적용 전 `targetExe.sha256`를 `95d8ed11…`로 갱신하고 기존 login 패치 엔트리의 sourceExeSha256도 맞춰야 한다(또는 95d8ed11 EXE 기준으로 매니페스트 재작성). **오프셋은 95d8ed11 EXE에서 검증됨.**
...
```
로비 메인(FUN_0051a370 state 0x16) — item1 이 disabled → (패치로 enable)
 └ item1 클릭 → state 0x19 : DAT_02217398 = 0x41, state → 0x2d
 ...
 └ default 케이스: (0x3f < state < 0x6a) → FUN_00594f20()  ← 캐릭터-엔트리 위저드
    (item2 오리지널은 동일 경로지만 state 0x40; 위저드가 0x40=원작추첨 / 0x41=커스텀생성 분기)
FUN_00594f20 위저드 (생성 분기):
    계정 조회(0x1000/0x1001) → 엔트리상태(0x1004/0x1005) → 폼(진영/출신/성별/이름/능력/얼굴/기함)
    → ★CommandGenerateCharacterCharge 0x1008 송신★ (송신 correlator FUN_004b78a0 kind 0xd → code 0x1008, gate 0x358375 LobbyLoginOK)
```

- 0x40(원작추첨)과 0x41(커스텀생성)이 **둘 다** `FUN_00594f20` 로 들어가되 **진입 state가 달라** 분기가 갈린다(정적 확정: 클릭 디스패처 case 0x19/0x1a + default 델리게이션 라인).
- **생성엔 SS 접속이 선행**된다(원작추첨과 동일). 서버는 위저드 도달 전 SS 핸드셰이크(0x2009→0x200a / CONNECT_SS / CERT_SS)와 0x1000/0x1004 응답을 이미 줘야 한다.
...
| item2 무반응 = 런타임 잠금 아님, 정적 disable(byte[2]=0) | **높음** (원바이트 + 잠금 분기가 메뉴 미접촉 확인) |
| item1 정적 disable 위치·최소 패치(offset 0x11ab3e 00→01) | **높음** (현재 EXE 원바이트 디스어셈블) |
| 실 enable 배열 = {1,0,0,0,0,1,0,1} (기존 {1,0,1,0,1,0,0,0} 정정) | **높음** (원바이트 8개) |
| index→라벨(item1=新キャラ작성, item2=오리지널, item3=삭제) | **높음** (클릭 디스패처 + 디버그 문자열) |
| item1 클릭 → state 0x41 → 위저드 → 0x1008 (원작 0x40/0x1006과 분기) | **높음** (case 0x19/0x1a + default 델리게이션) |
| 0x1008 요청/응답 packed wire | **높음** (기존 정적 RE + 실캡처 Reinhard/Lohengramm) |
| 패치 후 item1이 서버-데이터 경로로 재-disable될 가능성 | **낮은 리스크·미확정** (시퀀서 FUN_0051a370 내 재-enable 경로 없음 확인. 로비메뉴(id 0x50) 위젯을 만지는 외부 함수는 전수 확인 안 함 — apply 후 라이브로 확인 권장) |
| 능력 예산·진영/출신 열거값 범위 | **미확정** (폼 UI 규칙; 서버 권위 재검증으로 우회) |

## 6. 검증 게이트
- [ ] 매니페스트 `targetExe.sha256`를 95d8ed11…로 정합 후 `exe-patch validate`(dry) 통과.
- [ ] 패치 적용 EXE에서 **로비 item1(新キャラクターの作成) 클릭 가능**(라이브 — enable 확인).
- [ ] item1 클릭 → SS접속/인증 → 생성 폼 진입 → 0x1008 송신(프레임 캡처).
