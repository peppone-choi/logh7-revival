...
> ⚠️ **2026-07-11 정정 (정본 EXE 직접 디컴파일):** §2의 enable 배열 `{1,0,1,0,1,0,0,0}`(item2 enable/item1 disable)은 **오류**다. 정본 9c97de2a 디컴파일(`FUN_0051a370` case 0x16)은 **`{1,1,0,0,0,1,0,1}`** — **item1(커스텀 생성/0x1008)이 켜져 있고 item2(추첨/0x1006)는 꺼져 있다.** 따라서 "빈 계정 첫 캐릭터 = 추첨(item2)" 결론은 정본과 어긋난다. 정본 라이브 첫 캐릭터 경로는 **item1 커스텀 생성(0x1008)**. 전체 라이프사이클 정정·확정은 `docs/logh7-character-lifecycle-re.md` 참조.
