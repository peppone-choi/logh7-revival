...
2026-07-16 UnitShip operation: production obtains the 63-row SQLite ship catalog through `WorldCatalog.getShips()` but serializes only the live-safe first 19 rows in `0x030b`. The client body keeps count at +0, a four-byte header, `0x8c` records, a provisional numeric kind at record +0, model code zero at +6, and the ship key as the UTF-16LE cache label at +8/+0x0a. Sending 20 or more rows stalls admission. The 19-row boundary preserves dual-client world entry and authoritative movement, but it does not create `DAT_009d2fa8`; do not promote this transport slice to a proven ship-model join.
