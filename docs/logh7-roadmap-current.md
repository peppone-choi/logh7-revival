...
| M4 전략맵 커맨드 | 🟡 진행 | production SQLite의 `EnterWorld`·`MoveGrid` CQRS/UoW 연결 완료. 81개 중 factory 확인 2개, 79개와 ledger/CP/timers/jobs/outcome 미해결 |
| M5 전술·전투 | ⬜ 대기 | |
...
fresh gate는 targeted `97/97`, 전체 server `458 total / 456 pass / 0 fail / 2 pre-existing conditional skips`, Python live harness `16/16`, changed JS LSP error `0`, cell `0` 무변경 runtime probe다. M4 완료로 보지 않는다. PCP/MCP ledger, CP charge, timers/jobs, 실제 command outcome, `0x0327` 미확정 stock(zero-fill), disconnect `online=false`, PostgreSQL용 async bridge가 남았다.
