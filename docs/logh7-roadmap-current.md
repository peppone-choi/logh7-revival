...
> **TL;DR** M1~M3 라이브 완료(로그인→로비→월드·캐릭터), M4 전략맵은 구현 완료·관측 게이트 중. 다음 P0 = 격리 Wine/EXE 계보/증거 복구. 데이터 승격 규칙: 정본 = 코드·테스트 실행 기록(명령·산출물·계보 함께), 문서는 예보일 뿐.

작성일: 2026-07-06 (현행화: **2026-07-16**)
...
다만 현재 checkout에는 run9/run3/run5 evidence directory가 없고, 해당 run의 canonical/patch EXE 전체 hash와 lineage receipt도 없다. 직접 확인 가능한 EXE는 `bd192...` 계열뿐이며 문서의 `825635...` 문자열만으로 exact 실행본을 재구성할 수 없다. 따라서 아래 M0.5~M3 상태는 **완료 이력**으로 유지하되, live 수치와 M4 관측을 현재 release gate로 재사용하기 전에 격리 Wine prefix에서 증거를 복구·재실행해야 한다.
...
| M4 전략맵 커맨드 | 🟡 진행 전 관측 게이트 | production SQLite의 `EnterWorld`·`MoveGrid` CQRS/UoW와 `0x030b` 19행 admission은 구현됨. P0 Wine/계보/증거, P1 3면 correlation, P2 parser/cache/root/FSM을 먼저 닫아야 함. 81개 중 factory 확인 2개, 79개와 ledger/CP/timers/jobs/outcome 미해결 |
| M5 전술·전투 | ⬜ 대기 | |
...
M4 코드 변경 전에 아래 순서를 지킨다. PCAP/proxy는 **host network 계층**, 게임 화면·Win32 입력·Frida·D3D8 acceptance는 **격리 Wine 계층**으로 구분하며 서로를 대체 증거로 쓰지 않는다.

### P0 - 격리 Wine prefix, 정본 EXE 계보, evidence 복구

- 새 프로젝트/런 전용 32-bit `WINEPREFIX`를 강제하고 기본 `~/.wine` 접근을 금지한다.
...
- run9/run3/run5 evidence를 복구하거나 같은 exact hash로 재실행해 client/server/seed hash, packet/log, DB, screenshot, cleanup을 tracked redacted receipt로 남긴다.
...
범위는 `47900 → 47901` 양방향 무변형 proxy, exact EXE hash/address-profile 검사, 위 correlation schema, 단일 실행 recipe, cleanup/rollback이다. 완료 증거는 격리 Wine 1회 run에서 proxy 입출력 byte/hash 동일, server outcome join, direct-server control과 동일한 게임 동작, 종료 뒤 listener/process 0개와 prefix 외 변경 0개, tracked redacted receipt다.
...
- 리마스터 visual A/B는 Wine 계층에서 수행하고 protocol/FSM trace가 원본과 동일해야 한다.
...
- 실클라 Wine A/B에서 렌더링이 깨지지 않고 client/proxy/server protocol/FSM trace가 동일한 증거가 있다.
