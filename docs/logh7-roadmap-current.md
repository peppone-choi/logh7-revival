...
목표는 원본 클라이언트 + 자체 서버로 은하영웅전설 VII의 온라인 기능 전체를 되살리는 것이다. run9에서 M3의 두 계정 동시 월드 진입, 이동 브로드캐스트, 재로그인·서버 재시작 영속성, 정리를 8/8로 통과했다. 마일스톤은 M0.5/M1/M2/M3 `4/8 = 50%`지만, 무거운 M4~M7을 반영한 전체 작업량은 보수적으로 `30~40%`, 대표값 `35%`다.
...
| **M3 월드 진입·멀티플레이 영속** | ✅ 라이브 완료 | run9 직접 실행 5회. 두 계정 월드 진입, `0x0b01 → 0x0b07` B 반영, cell `2587` 재로그인·서버 재시작 유지, 프로세스·47900 정리. `.omo/live-qa/m3-two-client-persistence-1080p-cp932-20260714-run9/` |
| M4 전략맵 커맨드 | 🟡 진행 | production SQLite의 `EnterWorld`·`MoveGrid` CQRS/UoW 연결, `0x030b` live-safe 19행 UnitShip slice. 함선 마커/FSM 미복구. 81개 중 factory 확인 2개, 79개와 ledger/CP/timers/jobs/outcome 미해결 |
| M5 전술·전투 | ⬜ 대기 | |
...
fresh gate는 UnitShip 포함 targeted `132/132`, 전체 server `460 total / 458 pass / 0 fail / 2 pre-existing conditional skips`, Python live harness `16/16`, changed JS LSP error `0`, cell `0` 무변경 runtime probe다. `0x030b` 19행 run5는 두 클라이언트 월드 진입과 이동을 보존했지만 함선 마커 root null 및 post-warp HUD idle 실패를 남겼다. M4 완료로 보지 않는다. PCP/MCP ledger, CP charge, timers/jobs, 실제 command outcome, `0x0327` 미확정 stock(zero-fill), disconnect `online=false`, PostgreSQL용 async bridge가 남았다.
...
다음 순서는 M4 command authority/ledger/timer/job과 galaxy/fleet/facility/economy canon/data다. 이후 M5 전술·전투, M6 전체 한글화, M7 운영·리마스터로 진행한다.
