...
> **2026-07-20 실행 오버레이:** 현재 최우선 게이트는 GitHub #216 / Jira LOGH7-213의 15축 전체 인과 역기획 원장이다. 제안 설계는 [`logh7-causal-ledger-master-design.md`](logh7-causal-ledger-master-design.md)이며, 사용자 승인과 설계 PR 병합 전에는 제품 구현을 재개하지 않는다. 승인 뒤에는 D0→A01을 먼저 닫고 문서의 하드 DAG에 따라 #217~#231을 각각 독립 PR로 처리한다. 아래 P0→P1→P2→M4 기록은 폐기하지 않고 이 원장의 현행 증거·미해결 노드로 편입한다.
