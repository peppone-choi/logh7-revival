...
> 상태: **APPROVED-MERGED — PR #232 merge `ec6d9b52`; A01 구현은 `tools/causal-ledger/`의 `1.0.0` 계약을 소비**
> 설계 버전: `0.1.0`
...
- 현재 근거: `tools/causal-ledger/schema.json`과 validator/CLI가 통합 graph 계약을 구현한다. 고정 분모는 opcode 22, EXE 함수 11,593, UI/render source 8, data audit JSON 170이며 auxiliary 배열은 primary 분모 밖 개수와 사유를 report에 따로 기록한다.
...
## Wave 1 구현 상태 (2026-07-21)

**A02 input/UI/FSM, A04 protocol/session, A06 data/assets/P3, A09 failure/ops/lineage, A13 rights/redistribution** 구현·검증 완료.

### 구현 요약

| Axis | Nodes | Edges | Tests | Evidence |
|---|---|---|---|---|
| A02 input/UI/FSM | 27 (UI surface + Unknown/Blocked) | — | 21/21 pass | 클라이언트 UI 좌표 audit + WndProc/DirectInput enum 미확정 → Blocked node |
| A04 protocol/session/sync | 66 | 44 (opcode 22쌍 request/handler/response) | 7/7 pass | docs/logh7-opcode-coverage-current.md 22 rows |
| A06 data/assets/P3 | 170 (provenance/canonicality) | — | 13/13 pass | data-audit 170 records, P3 invariant 검증 |
| A09 failure/ops/lineage/safety | 18 (lineage + failure nodes) | — | 4/4 pass | docs/logh7-client-lineage-current.md + docs/logh7-wine-live-qa.md (documentary provenance only) |
| A13 rights/redistribution/clean-room | 4 (rights-disposition per source artifact) | — | 7/7 pass | rights default unknown (fail-closed), NO final judgment made |

### 공유 패턴 (모든 축)

1. `importSources(SOURCE_MANIFEST)` — A01 base 재부팅
2. axis-specific nodes/edges/evidence append
3. 각 axis node를 기존 A01 coverage record의 `targetNodeIds`에 연결 (orphan-node 규칙 만족, frozen per-source coverage count 미변경)
4. `validateLedger(ledger, { manifest: SOURCE_MANIFEST })` 통과

Deterministic (wall-clock·RNG·timestamp 고정), 2회 실행 byte-identical CLI 재생성, generated output delta-only under `tools/causal-ledger/generated/`.

### 검증 결과

- A01 regression: server/tests/logh7-causal-ledger.test.mjs 9/9 pass, exit 0 (frozen test 미변경)
- 모든 축 dedicated test: 각 exit 0, 총 55 pass
- CLI determinism: 2회 실행 SHA-256 ledger/report 일치
- node --check: 5개 모듈 + 5개 test 파일 모두 clean
- 초기 fabrication: A02 WndProc/VK hex 날조는 독립 검증에서 적발 → 실제 UI 기록 + explicit Blocked node로 정정, 재검증 통과
- 어떤 node도 canonical 승격되지 않음 (no reviewer/approvalRef)
- 실제 live behavior 주장 없음 (documentary provenance only)

### 신규 파일

- `tools/causal-ledger/axes/{a02-input-ui-fsm,a04-protocol-session,a06-data-assets-provenance,a09-lineage-failure-safety,a13-rights-and-redistribution}.mjs` (5 modules)
- `server/tests/logh7-causal-ledger-a0{2,4,6,9,13}.test.mjs` (5 test files)
- `tools/causal-ledger/generated/{a02-input-surfaces-states,a04-protocol-opcodes,a06-ledger,a06-report,a09-lineage-failure-safety,a13-rights-disposition}.json` (12 generated files)

### 다음 단계

- Delivery: single branch `peppone-choi/wave1-axes` → origin/main PR (merge 미승인)
