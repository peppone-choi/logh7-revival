...
## ✅ 성계 배치 라이브 렌더 — 이름·스펙트럼 마커 표시 (2026-07-11)

**전략맵에 실 성계가 마커로 렌더됨**(evdir `tools/live/_ev/stars-render2-015924/shots/99-final.png`, team-lead 육안 확인): ライガール·コブラヴェルデ·トリプラ 등 성계가 스펙트럼 색 글로우 별 + 성계명 라벨로 표시. "성계 데이터 없네" 해결.
- 구현: logh7-galaxy-placement.mjs(galaxy-systems.json 85성계 → 0x0313 팔레트 class=3 마커 + 0x0315 셀), world-session 0x0f02 스폰버스트가 emit. 커밋됨.
- 배치 불변식 서버측 검증: 0x0313 count=88(<101), 0x0315 Σrun=5000·rleLen 1012·5004B, 마커85(≤128), byte2∈{0..6,8}. 전부 OK.
- 자동 렌더 경로 **크래시 0**(client alive 내내, failCount=0, walkDone=1). 렌더는 안정.

**미해결(진행중, re-initdlg #): "初期化" 에러 다이얼로그 크래시** — 사용자 수동 상호작용(버튼/마커 클릭) 시 "初期化(초기화)…" 다이얼로그 뜨고 팅김. 자동런은 클릭 안 해 미재현. 가설: 미구현 명령/미초기화 서브시스템을 버튼이 건드림(카탈로그상 명령 핸들러 대부분 미구현). 단서: 클라가 렌더 후 headExp=0xf07 계속 대기(0x0f06→0x0f07 서버 미구현). re-analyst가 다이얼로그 문자열·트리거·미초기화 대상·초기화 조건 RE 중.

## ✅ M3 달성: 전략맵 렌더 라이브 확인 — BE 수정이 게이트 해제 (2026-07-11)

**원본 클라 + 자체 서버로 전략맵 렌더 최초 도달(리셋 전 사이클도 미완이던 지점).** 근본원인 = 0x0315 RLE-카운트 엔디안 회귀(LE→BE), 커밋 209d941a. liveqa-render 라이브 검증(evdir `tools/live/_ev/be-render-004316`):
- **0x315 디스패치**: count315=2(수정 전 전 런 0). recv큐 적재 enq315=2. **decrypt-fail 0**(수정 전 wiretap2는 0x315서 DECRYPT-FAIL+클라 크래시).
- **워크 완주**: dispWorkSeq에 0x315 포함 전 시퀀스 실행, walkDone=1(수정 전 0x0314 정지, seq=[]).
- **팅김 없음**: 클라 60초 내내 alive.
- **렌더 스크린샷 `shots/99-final.png`**(team-lead 육안 확인): 은하 그리드 성계맵 뷰포트 전체 렌더, 녹색 섹터격자+성운대, 하단 HUD(황제 초상·PCP/MCP·미니맵·"1,1グリッド"·커맨드바). NOW LOADING 걷힘.
- ★fade 프로브 주의: `clientBase+0x357e88`는 월드씬에선 "1.0=해제" 아님(다른 카운터). 렌더 판정은 walkDone=1+육안이 ground truth. 문서 fade 오프셋은 로비/특정씬 한정.
- 부수(블로커 아님): NO DATA·"退会済み"는 시드-우회 세션(월드/계정 데이터 없음) 아티팩트.

**다음: M4** — 전략맵 상호작용(성계 클릭·함대 이동·명령). 커스텀 생성(#69)은 서버측 완결(0x1008 128B OK 잠금), 로비 위저드 라이브 검증만 남음.

## 🎯 (경과) M3 유력 원인(옛 proven) = 0x0315 RLE-카운트 엔디안 회귀 LE→BE (2026-07-10)
...
2. **소켓 물리 도착** — ws2_32 recv 훅: 0x0307 응답(58824B 다중세그먼트) 수신→0x307 디스패치✓, 이어 **0x0315 응답(5032B) 프레임 도착**(hdr `13a6 00000000 0030 …`, lenField=5030, offset6=0x0030 정상). 파서 desync 없음.
3. **클라 파서 드롭** — 그런데 c315=0(디스패처 미호출), recv큐(+0x3552b8) empty. **0x0307(58KB) 처리 직후 다음 0x0315 프레임을 recv큐에 못 넣음.**

★와이어 실좌표 확정: 월드/어드미션 프레임 = `[u16BE len][4B 00 subheader][u16BE 0x0030@offset6][암호화body]`. (서버 모듈 주석의 offset2와 달리 code는 **offset6** — subheader4 때문. 테스트 decodeFrame이 이 좌표로 잠금.)

**남은 것: 클라 파서 드롭 분기 특정(stage-2 진행 중).** FUN_004b8850(수신적재) 훅 — 후보: checksum 불일치 / id 게이트(id≤client+0x20) / 크기 테이블. 특히 58KB 프레임 직후 파서 상태. 클라는 원본 EXE(수정 불가) → 밝혀진 분기 조건을 **서버 0x0315 emit이 만족**하도록 수정(예: id 윈도우/innerLen). 태스크 #76.

## 🎯 (경과) M3 블로커 3중 협공: 0x0315는 클라에 아예 안 착지 — 서버 전용핸들러 emit 경로 의심 (2026-07-10)
...
```
walkStep: 0x2, waitCount: 1, fade: 0(로딩 안 걷힘), walkDone: 0
queue: [{send: 0x314, exp: 0x315}]   ← 클라는 0x0315 기다림 (headExpCode=0x315)
```
- 0x0304/0x0306(0x0305/0x0307)은 **팝됨**(큐에서 사라짐, 클라가 0x0314까지 진행). 그러나 **0x0315만 미팝** — 서버가 0x0315를 코드(0x0315)·크기(`Buffer.alloc(0x138c)`=5004B, RLE ΣrunLen=5000 유효) 다 맞게 보내는데도.
- 즉 팝 실패는 그리드 malformed/크기 아님. **팝 조건 = 코드 일치 + owner-id(-1/self)**(re-loadgate). 0x0305/0x0307은 팝하는데 0x0315만 안 하니, **0x0315(그리드) 핸들러가 owner-id를 -1로 고정 안 할** 가능성(char/base 핸들러와 다른 경로) → 서버 0x0315 payload에 owner-id=-1 필드가 필요하거나, 별도 조건.
- 사용자 관찰 "로딩에서 팅김": 측정 런은 alive(정지)였으나, 큐 desync/타이밍에 따라 크래시로도 나타날 수 있음. 핵심 = 0x0315 미팝.
- **다음**: re-loadgate가 클라 0x0315 dispatcher-tail 팝 조건(owner-id 고정 여부, 0x0305 대비 차이) 확정 → 서버 0x0315를 그 조건 맞게.

## 🔬 (경과) 시드-우회 검증: 로비 우회 성공, 워크 0x0314 정지 (2026-07-10)

**시드 store로 로비 잠금 우회 성공(`server/data/m3-seed-20260710-180701`): 로그인→로비(캐릭터 표시)→SS접속→월드 진입까지 도달.** 그러나:
- **클라 수신 프레임: 0x0304/0x0306/0x0314 후 정지**(0x0312/0x030a 등 다음 워크 요청 무송신). 워크 응답 완비(3db04f2c)+핸드셰이크(521a3bd9)에도 **동일 0x0314 정지**.
- 서버는 0x0314에 0x0315 응답함(09:09:18.470, kind:admission). **코드=0x0315 정확, 크기=`Buffer.alloc(0x138c)`=5004B 정확**(클라 사이즈 테이블 일치). 그런데도 클라가 step2(0x0312)로 안 넘어감.
- 즉 **코드·크기가 맞는데 워크 큐가 0x0315를 팝 안 함.** 남은 후보: 응답 프레이밍(msg32 wrapping)이 walk 큐 매칭과 안 맞나 / world-enter 배치(0x0323/0x0325 unsolicited)가 큐 desync / owner-id 필드 / 워크가 애초에 FUN_004b76e0로 안 돌고 있나.
- **결정적 다음 실측**(re-loadgate 설계): 0x0314 정지 시점에 `clientBase+0x357ec0`(대기 카운트)·`0x357ec4`(큐 헤드 send 코드)·`0x357ec8`(**기대 응답 코드** — 클라가 정확히 뭘 기다리나)·`0x357e88`(페이드) 읽기. 0x0315면 프레이밍/전달 문제, 다른 코드면 워크 순서 오해.

## ⚡ (경과) NOW LOADING 정답(RE) + 로비 잠금 = 실 캐릭터생성 블로커 (2026-07-10)

**두 결정적 발견:**

### (1) NOW LOADING dismiss = 정적정보 워크 완주 (re-loadgate, `docs/logh7-now-loading-gate-re.md`)
- dismiss = 로딩 페이드 `clientBase+0x357e88`→1.0. 이 페이드는 **정적정보 워크(FUN_004b76e0)가 완주**해야 램프(+0.06). 워크는 16요청을 **하나씩**(대기 큐 0x357ec0==0 게이트) 보내고 각각 **정확 코드+크기** 응답 받아야 진행.
- 라이브 정지(0x0304/0x0306/0x0314 후) = **0x0315 응답 대기**. 서버가 워크 전 요청(0x0314→0x0315, 0x0312→0x0313, 0x030a→0x030b, 0x0310, 0x030e, 0x031c, 0x0308, 0x030c, 0x0300, 0x0f00, 0x0f02)에 정확 응답해야 완주.
- **objTable 충만은 이 게이트와 무관**(0x0b0a 그리드엔터가 채움). ← 그동안 objTable/flagship/엔디안에 매달린 게 헛다리. srv-walk가 워크 응답 구현 중.

### (2) 실 캐릭터 생성 블로커 = 로비 잠금 (liveqa-hs 라이브, M2 문서 정정)
- **빈 계정(count==0) 로비가 「セッションサーバー不具合」로 잠김. 이 상태서 item2 추첨 버튼도 클릭 무반응**(정적 enable item2=1인데도 런타임 잠금이 오버라이드). 0x2006/0x2009/0x1006 요청 전무.
- **M2 문서의 "item2=첫 캐릭터 escape-hatch"는 거짓**. m2-gate-confirm 성공은 store.json에 캐릭터 **수동 시드**로 count=1 만들어 잠금 우회한 것뿐 — 실제 인게임 로터리/생성은 **어느 런에서도 성공한 적 없음**.
- 로비 잠금 = `DAT_02216c88==0`(0x2004 body[0]=count). **커스텀 생성(item1)·추첨(item2) 둘 다의 전제 = 이 잠금 해제.** re-custom이 잠금 해제 조건(순수 count냐 세션서버-available 플래그냐) RE 중.
- 온보딩 후보: 서버 스타팅-캐릭터 프로비저닝(count≥1로 잠금 해제) 후 로비에서 item1/item2 사용.
- **워크/렌더 검증은 캐릭터 시드로 잠금 우회해 진행 가능**(월드 진입 후는 잠금과 독립).

## 🧱 (경과) M3 렌더: objTable+그리드셀 충족해도 NOW LOADING — 로딩 트리거는 워크 완주로 확정됨 (2026-07-10)
...
### ✅ 이 세션에서 완주한 것 (실클라 라이브 검증)
로그인(0x7000) → 로비(0x2000→0x2001, count≥1 완전작동) → 캐릭터 선택 → SS 3계층 핸드셰이크(0x2009→0x200a msg32→conn3 재접속→0x0200→0x0201→0x0205) → world-enter(0x0206/0x0204/0x0323/0x0325) → 어드미션 핸드셰이크(static-info 고정크기 프레이밍, 0x0304~0x0f02 req-resp) → **전략맵 렌더러 진입(크래시 해소, 55초+ 생존)**. 유닛 202/202.

### ⛔ 최종 단일 블로커: NOW LOADING → 전략맵 플립
- **진짜 격차(라이브 대조)**: 서버가 **KNOWN-SUCCESS 트레이스의 정적정보 데이터 체인을 방출하지 않음.** 성공 시 서버→클라: `0x0200/0x0201 → 0x0300..0x031f 정적/정보 전범위 → 0x0313×2(섹터그리드)/0x0315 → 0x0323×2/0x0325×2 → 0x0b09/0x0b0a → 0x0f02(월드초기화) → 0x0f06/0x0f07(재동기)` (`docs/reference/restored-from-git/logh7-live-world-entry-2026-06-23.md:9`).
- **현재 서버가 한 번도 방출 안 한 코드**: `0x0201, 0x0301, 0x0313, 0x031d, 0x031f, 0x0f02, 0x0f06, 0x0f07`. 클라는 이 정적정보(섹터그리드 0x0313, 베이스 0x031d, 성계 0x031f)로 맵을 채우고 0x0f02/0x0f06/0x0f07로 렌더 마무리 → 없으니 NOW LOADING 대기.
- **왜 push 순서가 답이 아니었나**: 순서(0x0325/0x0323 begin/end 사이)는 맞췄으나 클라가 필요로 하는 **데이터 레코드 자체가 부재**. 축이 너무 좁았음.

### 반증된 가설 (재시도 금지)
- send-ring 상관/pre-push (라이브 반증 — pre-push 유무 무관 동일 정지)
- world-ready push 순서 (계약순서 맞춰도 정지 — 데이터 체인 부재가 진짜 원인)

### 다음 접근 (데이터 체인 축)
1. **restored 성공 문서에서 전체 정적정보+월드초기화 체인 스펙 확보**(레코드 목록·순서·포맷·타이밍): `logh7-live-world-entry-2026-06-23.md`, `logh7-live-validation-2026-06-30-grid-info.md`, `SESSION-HANDOFF-2026-06-15-live.md`, `logh7-inworld-progress.md`.
2. **한 번에 구현**(증분 추측 금지): 누락 레코드(0x0301/0x0313/0x031d/0x031f + 0x0f02/0x0f06/0x0f07) 빌더를 문서 근거로 배선, 성공 트레이스 순서/타이밍대로. 갤럭시 GREEN·베이스 데이터 활용.
3. live-qa 재검증.
- 원본 성공 트레이스 raw(`.omo/ui-explorer/live3-auto/trace.jsonl`)는 리셋 때 삭제 — restored 문서 서술이 유일 근거.

### 증거 디렉터리 (이 블로커까지 라이브)
`m3-worldready-20260710-0558`(push 발화), `m3-contractorder-20260710-0617`(계약순서, responseCount:6, 여전히 NOW LOADING).
...
### 확정된 사실 (라이브 재현)
- 로그인→로비→캐릭터선택→SS 핸드셰이크→월드 진입→어드미션 핸드셰이크는 **전부 실클라 통과**. 크래시 없음(0x0323 실 캐릭터로 오브젝트 테이블 채워 렌더러 크래시 해소, 55초+ 생존).
- reactive 라우팅 정상(요청↔응답 순차 배수 확인). world-enter 배치 = [0x0206,0x0204,0x0323,0x0325] 코드대로 동작.

### 반증된 가설
1. **send-ring 상관/pre-push 이론(반증)**: pre-push 제거(8코드→4코드) 후에도 **동일 지점 정지**. `m3-final-20260710-0529` vs `m3-stratmap-render-20260710-0441` 트레이스 대조 — 배치 코드 수만 다르고 정지점 동일(0x0314 후). send-ring이 NOW LOADING 정지의 원인이 아님. (단 reactive 수정 자체는 클라 요청-응답 모델과 정합하므로 유지.)

### 결정적 새 단서 (다음 조사의 핵심)
- **오브젝트 테이블/그리드를 채우자 클라 워크가 14요청→3요청으로 줄었다.**
  - 빈 0x0323+빈 그리드: 클라가 14요청(0x0304~0x0f02) 걸어가다 **렌더 크래시**(빈 오브젝트 테이블).
  - 실 0x0323+유효 그리드: 클라가 **3요청(0x0304→0x0305, 0x0306→0x0307, 0x0314→0x0315)만** 하고 정지, 크래시 없음.
- 즉 **채워진 상태가 클라를 다른 로더 경로로 보내 조기 정지 + 대기**시킨다. 클라가 3요청 후 무엇을 기다리는가가 핵심 — **서버 push(비요청) 대기 가능성**.

### 남은 가설 (우선순위)
1. **서버 push 대기**: 클라가 핸드셰이크 후 서버 주도 push(0x0f02 NotifyWorldLocation? 0x0325 실 유닛/함대? world-ready 통지?)를 기다림. render-interaction-contract는 "조기 0x0f02 push는 클라 크래시" 경고 → 특정 시점의 push가 필요. **0x0325 유닛 테이블이 최소 count만이라 클라가 실 유닛/함대 데이터를 대기할 수 있음.**
2. **0x0305/0x0307 빈 walker body**: `buildEmptyWalkerInner` 풀사이즈 0채움이나 바디 포맷 미확정. 클라가 count=0으로 읽어 워크 조기 종료 가능(단 0채움이라 count=0은 자연스러움).
3. **0x0315 빈 그리드 + 0x0313 팔레트 미동반**: 클라가 0x0312(팔레트) 요청 없이 0x0314만 보냄 — 빈 그리드라 렌더러가 완주 신호 못 받을 가능성.

### 권장 다음 접근 (축 전환)
- **정적 RE 반복 자제**(send-ring 오판 전례) → **동적/차분 분석**: (a) 14요청 런과 3요청 런의 클라 상태 차이를 로더 `FUN_004b76e0` 스텝 카운터(`DAT_007cd020`) 관점에서 대조, (b) 3요청 후 클라가 폴링하는 정확한 플래그/버퍼가 무엇이고 어느 서버 레코드(push 포함)가 그걸 세팅하는지 확정. (c) 실 유닛/함대(0x0325)와 갤럭시 systemId(0x0315 non-zero cell)를 채운 뒤 거동이 바뀌는지 라이브 A/B.
- 또는 이전 사이클(리셋 전, 전략맵 렌더 성공)의 `restored-from-git/logh7-inworld-progress.md` 전체 성공 트레이스를 재정독해 3요청 후 정확한 다음 프레임(서버가 무엇을 push/응답했는지)을 문서에서 확보.

### 세션 성과 (이 블로커까지)
로그인~월드 진입 프로토콜 전 계층(SS 3계층 핸드셰이크, 어드미션 고정크기 프레이밍, world-init req-resp, 오브젝트 테이블/그리드 크래시) 재구현·라이브 검증. 유닛 198/198. 남은 단일 블로커 = NOW LOADING→전략맵 최종 플립(위 대기 원인).
