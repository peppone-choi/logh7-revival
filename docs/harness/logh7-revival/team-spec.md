...
원본 `G7MTClient.exe`를 1차 제품·호환성 오라클로 유지하면서 정적 RE, 실행 환경별 client runtime, wire interception, 권위 서버를 같은 증거 체계로 결합한다. native Windows는 직접 실행하고 macOS/Linux만 격리 Wine을 사용한다. 리마스터와 장기 재이식은 병렬 포트폴리오로 운영하되 legacy M4 복원을 방해하거나 대체하지 않는다.
...
- **상태 자원:** 선택된 client runtime 작업 영역(Wine mode의 prefix 포함), install copy, live DB, client/server ports, GUI는 단일 `live` owner가 직렬 소유한다.
...
- `request.json`은 `runMode: regression|recovery-baseline`과 `runtimeMode: native-windows|wine`을 필수로 가지며 run 시작 뒤 바꾸지 않는다. 지원하지 않는 host는 run을 만들지 않고 blocked receipt만 남긴다.
...
| --- | --- | --- | --- | --- |
| `client` | 선택된 runtime의 동적 RE, Frida/debugger, Win32/GDI/D3D8, parser/cache/root/FSM | exact EXE profile, static hypotheses, live scenario | `branches/client.md`, client events | `static` |
...
| `live` | runtime routing, UI/game acceptance, ports/PID/cleanup/rollback | `$logh7-wine-live-qa`, runnable server, scenario | `branches/live.md`, evidence manifest | orchestrator + affected producer |
...
- `sys.platform`을 먼저 기록한다. native Windows에서는 Wine 입력·명령 없이 직접 실행하고, macOS/Linux에서는 격리 Wine gate를 적용하며, 그 외 host는 blocked다.
- host PCAP/proxy 판정과 선택된 runtime의 화면·입력·D3D8·audio 판정을 분리한다.
- 한 run이 runtime 작업 영역, install copy, DB, ports, GUI를 독점하며 기록된 PID만 정리한다.
...
- original fallback, provenance, hash guard, `enabled: false` 기본, rollback이 있는 overlay만 legacy client runtime A/B로 보낸다.
...
2. **Client runtime:** 선택된 runtime에서 인자·반환값·memory/FSM과 natural UI output을 관측한다.
...
### P0 — 실행 환경별 client runtime·client lineage·evidence 복구

Producers: `static`, `client`, `live`. 정적 inventory는 병렬 가능하지만 exact lineage, run mode, runtime mode가 확정되기 전에는 client 실행을 금지한다. native Windows는 직접 실행하며 Wine 입력을 받지 않고, macOS/Linux는 저장소 밖 run 전용 prefix, 명시적 `win32|wow64` mode, exact Wine toolchain을 요구한다.

두 경로를 구분한다.

- `regression`: 기존 run9 원본 또는 검증 가능한 redacted evidence index를 입력으로 요구한다. 없으면 fail-closed한다.
- `recovery-baseline`: run9가 없고 exact lineage/hash/PE metadata/sentinel이 검증된 경우에만 새 runtime 작업 영역에서 run9-equivalent evidence를 재생성한다. 해당 run verdict는 `provisional|recovered`뿐이며 `pass`가 아니다.

완료 gate:

- canonical/patch EXE full lineage, PE metadata, sentinel, backup/rollback이 검증됨.
- 공통 environment receipt에 host platform, `runtimeMode`, client path/hash, D3D8/locale/audio/input 설정이 있음. Wine mode는 추가로 absolute toolchain과 repo 밖 전용 absolute prefix/drive/registry를 기록하며, native Windows mode에는 Wine 필드·명령이 없음.
...
exact lineage 또는 선택된 runtime의 필수 gate가 없으면 두 경로 모두 시작하지 않는다.
...
- host network verdict와 선택된 client runtime의 game verdict가 각각 존재하고 cleanup/rollback이 닫힘.
...
- P0 exact lineage/runtime gate failure는 downstream 전체 blocker다. Wine toolchain 부재는 Wine mode에서만 blocker이며 native Windows에 Wine 설치를 요구하지 않는다. run9만 누락된 경우 normal regression은 blocker지만 `recovery-baseline` 예외를 사용할 수 있다.
- P1의 client/wire/server 중 하나가 누락되면 나머지 두 면의 결과만 좁게 보고하고 cross-layer claim을 금지한다.
- stateful live run failure는 같은 runtime 작업 영역/port에서 병렬 재시도하지 않는다. cleanup을 먼저 증명한 뒤 새 `RUN_ID`로 재시도한다.
...
미래 Windows x64 candidate acceptance는 legacy `$logh7-wine-live-qa`와 분리한다. native Windows에서는 직접 실행하고 Wine host에서는 candidate 전용 Win64 prefix를 쓰며, 어느 경우든 별도 ports, PID ledger, evidence namespace를 사용하고 candidate verdict를 legacy regression pass로 합치지 않는다.
...
- 관련 자동 test와 선택된 runtime의 실제 client natural-output evidence를 분리한 결과.
- exact hashes, PCAP/log/DB/screenshot index.
- process/port/runtime 작업 영역/registry/EXE cleanup·rollback receipt.
- 누락·미검증·부분 실패를 포함한 `synthesis.md`.

자동 test, process exit code, host proxy, client screenshot 중 하나만으로 gameplay 완료를 주장하지 않는다.
