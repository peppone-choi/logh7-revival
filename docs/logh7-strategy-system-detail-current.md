...
> [!warning] 현재 판정
> B71이 통과했다. 원본 UI 조작만으로 `職務権限カード → 제국 Captain kind 59 → 寄港(factory 0x2d) → panel kind 5 → base 70 → phase 0/1 → FUN_0057aa90`이 한 타임라인에서 닫혔다. 이것은 **경로와 계약의 자연 출력 증명**이며 상세 데이터 렌더 완료가 아니다. 화면의 情報 패널 필드는 여전히 공란이다. 서버 `0x0327`이 base ID만 싣고 경제·창고 스칼라를 비워 두는 보수 계약 때문이다. 이후 정적 RE로 `FUN_0057aa90`이 읽는 창고 캐시(`base+0x3e098c`) 오프셋과, 상세 데이터가 뜨는 자리가 拠点 모달이 아니라 전략맵 좌상단 정보패널임을 확인했다(아래 「상세 렌더 표면」). 현재 열린 경계는 그 오프셋의 엔디안·tag 의미와 kind `5/0x11` 진입 계기를 라이브로 확정하고 정본 데이터를 결합하는 일이다.
...
- **성계 관련 확인됨:** 서버 선택, 정적·동적 cache, `unit[0]+0x40=70`에서 이어지는 ID `70` lookup, factory router와 kind `5/0x11` 생산 경로, `FUN_00579e60 → FUN_0057aa90` 호출 경로. B71에서 Captain kind `59` 자연 선택, factory `0x2d`, panel kind `5`, phase `0/1`, `0x0327`, `FUN_0057aa90(base 70)`을 한 런에서 라이브로 이었다.
- **인접 C002로 확인됨:** `0x0323` seat/card와 `PLAYER_INFO+0x270`에서 만들어지는 직무권한 카드·유닛 선택 행, 그 행의 `FUN_005015f0` admission 거부.
- **아직 미해결:** `FUN_0057aa90`이 실제로 읽는 상세 필드 오프셋의 고정, 그리고 `0x031f/0x0321/0x0327`의 경제·시설 스칼라와 정본 데이터의 결합. 화면 상세 필드는 여전히 공란이다.
...
### 2026-07-13 상세 렌더 표면 정적 RE

renderer `FUN_0057aa90`(VA `0x57aa90`)가 실제로 읽는 소비 표면을 정적 RE로 좁혔다. 이 함수는 클라이언트 창고 캐시(`base + 0x3e098c`, 768바이트)에서 필드를 읽어 **전략맵 정보패널 뷰 kind `5`/`0x11`**의 情報 슬롯(slot 0/1/2/10, 드로우 `FUN_00646616 → FUN_0057b7b0`)에 그린다. 상세 데이터가 뜨는 자리는 拠点 모달이 아니라 전략맵 좌상단 정보패널이다.
...
kind `5`를 하드코딩하는 유일 지점은 `0x58be72`의 `mov [esi+0x28],5`이며, 이 지점은 클래스 **SendOrganizeDataCommand**(組織/編成 데이터, 문자열 `0x78c858`)에 속한다. 따라서 kind `5`(基地+駐留艦隊 패널)는 함대 편성류 전략 명령을 실행할 때 열린다. B71의 `寄港`도 같은 명령 인프라(베이스 vtable `0x670274`, 생성자군 `0x580000`–`0x58c000`)의 한 인스턴스다. factory `0x2d`가 `SendOrganizeDataCommand`와 같은 것인지, `0x11` 경로가 어떤 명령인지는 grade-c로 미확정이다. 앞서 render-view 판정에서 이 진입을 "vtable 간접"으로 본 표현은 정정됐다. 직접 호출자 3곳이다.
...
| `0x0326` phase 1 요청 | 클라이언트 → 서버 | `8` bytes | — | 오프셋 `0`의 **u32BE** base ID | 뒤 4바이트의 의미 |
| `0x0327` phase 1 상세 | `0326 → 0327` | `0x300` = 768 bytes | fixed compact big-endian record | 요청한 base ID. B71에서 body 첫 4바이트 `00000046`으로 확인 | 경제·창고 상세 스칼라. 값은 `0`/empty이며 이 때문에 화면 필드가 공란이다 |
...
- `0x0327` 역시 base ID만 싣는다. B71에서 renderer까지 자연 도달했지만 화면 필드가 공란인 직접적인 이유가 이것이다.

B71이 소비자를 자연 경로로 깨웠으므로, 다음 순서는 **renderer가 실제로 읽는 오프셋을 라이브로 고정한 뒤 그 오프셋에만** 증거가 있는 원천 데이터를 결합하는 것이다. 오프셋을 고정하기 전에 payload를 넓히면 UI 진전과 무관한 변수를 더해 진단을 흐린다. 시설을 임의 생성하지 않는다.
...
당시 verdict의 `factory41Granted`와 이를 first missing으로 본 판정은 폐기한다. `0x41`은 whitelist 밖이라 제품 권한이 아니다. 옛 parser도 runtime outer count `2`를 넘어 읽어 garbage category `15`, raw count `229`를 만들었다. count over-read와 phase 계측은 `1f683df0`에서, factory `0x41` 전제는 route-aware tracer 커밋 `319a1ba9`에서 각각 고쳤다.

### B71 — 자연 출력 경로 통과

근거 디렉터리: [`m3-system-output-B71-captain-0x2d-natural-20260713-run8`](../.omo/live-qa/m3-system-output-B71-captain-0x2d-natural-20260713-run8/)

> [!success] B71 판정
> `b71-verdict.json`의 `pass`는 `true`다. 9개 관문 `factory2dGranted`, `factory2dSelected`, `handler2dEntered`, `panelKind5`, `selectedBaseId=70`, `phase0Seen`, `phase1Seen`, `rendererCalled`가 모두 섰고 클라이언트는 끝까지 생존했다. QA command injection은 쓰지 않았다.

자연 UI 조작만으로 통과한 경로는 다음과 같다.
...
`決定`(이동 확정)은 누르지 않았다. 클라이언트 창고 캐시 `0x3e098c`의 `warehouseBaseId`는 스냅샷 `20/20`에서 `70`이었고, 서버 트레이스의 `selectedBaseId`도 `70`, `0x0327` 응답 body 첫 4바이트는 `00000046`이었다. `factory41Observations`는 grant·selected·handler 모두 `false`다. 정리 뒤 G7 프로세스와 TCP `47900` listener는 각각 `0`이었다.

**B71이 증명하지 않은 것:** 화면의 情報 패널 필드는 여전히 공란이다([`shots/06c-base-row-after.png`](../.omo/live-qa/m3-system-output-B71-captain-0x2d-natural-20260713-run8/shots/06c-base-row-after.png)). B71은 배선(wire → cache → renderer)과 계약의 자연 출력 증명이며 상세 데이터 렌더 완료가 아니다. 두 항목을 합치지 않는다.

QA 하네스에서 교정한 함정 세 가지 — 카드 행 역순, 명령 버튼 역순(`ワープ航行`이 `0x2b`), 카드 로딩 레이스 — 는 하네스 이슈이며 제품 병목이 아니다.

### 해소된 `0x0326` 요청 base 엔디안 — `b1bf8bc0`

원본 클라이언트 요청 body의 실바이트는 `reqBodyHex = 0000004600000000`(8바이트)이고, base는 오프셋 `0`의 **u32BE**(`0x46` = `70`)다. `0030-decoded`가 보고하는 `innerLen: 10`은 inner 전체, 즉 코드 2바이트에 body 8바이트를 더한 값이다.

서버가 이 필드를 u32LE로 읽어 `1174405120`을 얻었고, 그 값으로는 catalog 조인이 실패해 base `0`인 빈 `0x0327`을 돌려보냈다. 그래서 클라이언트 창고 캐시가 `0`이 되고 상세 패널이 공란이었다. run7이 해석 후보 다섯 가지를 노출했고 BE 판독만 조인에 성공해 관측으로 확정했다. 실측 바이트는 골든 벡터 테스트로 고정했으며 서버 전체 테스트는 마커 골든 벡터 테스트를 포함해 `398` 통과다(직전 `396`).
...
전송·수신·디스패치·cache, `unit[0]+0x40=70`, generic info factory 생산 경로, 권한 route, `0x0326` 요청 base 판독까지 모두 닫혔다. B71이 다음 경로를 자연 UI 조작만으로 라이브 증명했다.

```text
서버 authorityCards
→ 0356 {kind, spot}
→ 0305/0307 kind별 factory 목록
→ Captain kind 59의 0x2d (寄港)
→ FUN_00582060 → FUN_00570eb0(kind 5)
→ selected record +8 = base 70
→ FUN_00579e60 phase 0/1
→ 031f + 0327 (base 70, u32BE 요청)
→ FUN_0057aa90
```
...
**현재 열린 경계는 상세 필드 자체다.** 배선이 renderer까지 닿았는데도 화면의 情報 패널은 공란이다. `0x0327`이 base ID만 싣고 경제·창고 스칼라를 `0`/empty로 두기 때문이며, 이는 위 「왜 현재 `id-only`, `facilities=0`인가」의 보수 계약이 그대로 관철된 결과다. renderer가 읽는 오프셋 자체는 정적 RE로 확인했으므로(위 「상세 렌더 표면 정적 RE」), 남은 일은 세 갈래다.

1. **정밀 프로브로 kind 경로와 factory 정체를 라이브 확정한다(grade-c → a).** `FUN_00577e70` 진입(모듈 베이스 + RVA `0x177e70`)에 함수 경계 훅을 붙여 `param_1`(`[esp+4]`)과 복귀 주소(`[esp]`)를 로깅한다. 복귀가 `0x571b15`면 가변 kind 경로이며 `param_1`이 `5`/`0x11`인지 확인한다. 보강으로 `0x171870` 진입 훅에서 `[ecx+0x28]`을 로깅해 열릴 kind를 사전 캡처한다. 전략 명령 메뉴(편성·`寄港` 등)를 순차 조작해 어느 명령이 kind `5`/`0x11`을 발화하는지와 factory `0x2d` 동일 여부를 확정한다. mid-function 인라인 훅은 금지한다.
2. **마커를 올바른 패널에서 재검증한다.** B71 `寄港` → base `70` 경로를 마커 ON으로 재현하되 拠点 모달이 아니라 전략맵 좌상단 정보패널을 관측해 `66/100/200/1234`로 엔디안과 tag 의미를 확정한다.
3. **정본 창고·경제 데이터를 결합한다.** 오프셋과 의미가 확정된 뒤에만 `0x031f`의 소유·경제 스칼라, `0x0321`의 시설 종류·레벨·HP·생산성, `0x0327`의 phase 1 상세 스칼라를 정본 provenance로 채운다.

### B72 마커 실험 — 보류

renderer 오프셋에 서버가 구별 가능한 값을 실어 라이브 대응을 확인하려 했으나 보류했다. 서버 QA 게이트 `LOGH_QA_WAREHOUSE_MARKER=1`(`warehouse-record.mjs`, 게이트 뒤 격리·테스트, **미커밋**)로 `0x0327`에 구별값(재고 합 `66`, 카테고리 tag `0x10=100`·`0x11=200`, 스칼라 `1234`)을 실었지만 라이브 검증은 실패했다. 원인은 둘이다. 첫째, base 선택 플레이크로 B72 5회 시도에서 카메라가 세션마다 이동해 `ヴァルハラ` 별을 고정 좌표로 맞추지 못했다(run1 `(418,388)`, run2 `(595,385)`, run3 `(505,388)`). 둘째, 관측 패널 오류로 拠点 모달의 情報 칸을 봤지만 창고 상세는 그 모달이 아니라 좌상단 정보패널에 렌더된다. run8 완전 통과에서도 모달 情報 칸이 공란이었던 것이 그 증거이며, 모달은 renderer를 mount하지 않는다. 情報 슬롯(`FUN_0057b7b0`)은 월드·전략 로드 시점에만 발화하고(slots 0,1,2,10) 로드 후의 맵·버튼 클릭에는 발화하지 않는다.

권한카드 브리지의 리뷰 후보 4건 — 명시적 `authorityCards: []`의 의미, DB 교체 transaction, grant/revoke API, `seat count` 테스트 명칭 — 은 아직 열려 있다. 자세한 재개 순서는 [[logh7-m4-strategy-system-detail-handoff-2026-07-13|M4 전략 성계 상세 핸드오프]]를 따른다.
