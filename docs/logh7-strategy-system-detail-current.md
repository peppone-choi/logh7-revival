...
서버가 이 필드를 u32LE로 읽어 `1174405120`을 얻었고, 그 값으로는 catalog 조인이 실패해 base `0`인 빈 `0x0327`을 돌려보냈다. 그래서 클라이언트 창고 캐시가 `0`이 되고 상세 패널이 공란이었다. run7이 해석 후보 다섯 가지를 노출했고 BE 판독만 조인에 성공해 관측으로 확정했다. 실측 바이트는 골든 벡터 테스트로 고정했으며 전체 테스트는 `396/396` 통과다.
...
**현재 열린 경계는 상세 필드 자체다.** 배선이 renderer까지 닿았는데도 화면의 情報 패널은 공란이다. `0x0327`이 base ID만 싣고 경제·창고 스칼라를 `0`/empty로 두기 때문이며, 이는 위 「왜 현재 `id-only`, `facilities=0`인가」의 보수 계약이 그대로 관철된 결과다. 다음 두 단계가 남았다.

1. `FUN_0057aa90`이 실제로 읽는 상세 필드 오프셋을 라이브로 고정한다. B71 경로가 renderer를 자연 호출하므로 같은 시나리오에 read 계측만 붙이면 된다.
2. 고정된 오프셋에만 `0x031f`의 소유·경제 스칼라, `0x0321`의 시설 종류·레벨·HP·생산성, `0x0327`의 phase 1 상세 스칼라를 정본 데이터와 결합한다.
