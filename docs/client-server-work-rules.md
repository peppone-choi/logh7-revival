...
- 서버는 테스트 때만 로컬로 직접 실행한다. 실제 운영 서버는 AWS의 Docker 컨테이너로 실행한다.
...
- 로컬/원격 서버 실행 스크립트 작성
- AWS Docker 이미지 빌드, 실행, 로그 확인 절차 작성
...
- 로컬 직접 실행: 개발 중 단위 테스트, 통합 테스트, 빠른 HTTP 확인에만 사용한다.
- macOS 내장 Linux 직접 실행: 추출, 분석, 패킷 캡처, 임시 디버깅에만 사용한다.
- AWS Docker 실행: Windows 클라이언트가 접속할 실제 서버 검증과 장기 실행에 사용한다.

AWS Docker 운영에서 반드시 기록할 것:
...
- 테스트 목적이 아닌 서버를 로컬 직접 실행 상태로 방치하지 않는다.
- 실제 접속 검증용 서버를 Docker 없이 AWS에서 직접 실행하지 않는다.
...
2. AWS Docker 컨테이너가 같은 host/port 또는 리다이렉션된 대상에서 실행됨.
3. 컨테이너 로그가 Windows 클라이언트 요청을 기록함.
