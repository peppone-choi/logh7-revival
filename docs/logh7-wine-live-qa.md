...
**하네스 first-char-drop 수정(2026-07-17, macOS invalid-credentials 원인 해소):** 첫 글자 누락(`inei00`→`nei00`)의 원인은 QA 하네스 `tools/logh7_ui_explorer.py`(제품 버그 아님). 정밀화된 원인 = **창 세션의 첫 `KEYEVENTF_UNICODE` 주입이 삼켜짐**(이후 unicode 문자는 정상; 포커스/포그라운드·필드 클릭 문제 아님 — 라이브로 재현). v1(`313b666e`, SHIFT warm-up)은 SHIFT가 VK라 unicode 파이프라인을 워밍 못 해 **라이브 FAIL**(증거 `_workspace/liveqa-20260717-winnative-reverify/`). v2(`6c593202`) = `_build_type_sequence`가 진짜 문자 앞에 자기상쇄 `[unicode 더미 'x'][VK_BACK]`를 prepend → **라이브 PASS**: `inei00` 1회 온전 입력, 로그인→로비 end-to-end, 서버 authOk (증거 `_workspace/liveqa-20260717-winnative-reverify2/`). first-char-drop 종결. macOS login-ng도 같은 메커니즘이라 해소됨(macOS 실기 재확인은 별도).
