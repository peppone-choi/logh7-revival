...
> **2026-07-09 갱신:** M1(로그인~로비)은 라이브 증거로 완료. 갭 1 해소(아래 취소선), 다음 임계경로는 캐릭터 생성 → 월드 진입(M2/M3).

1. ~~**실클라 라이브 로그인 성공 증거** (0x7001/0x0200 계열 세션 OK까지 E2E)~~ ✅ **달성 2026-07-09** — `.omo/live-qa/m16-login-lobby-20260709-2201/` (로그인 conn1 + 로비 conn2 0x2001 진입, 로비 화면 렌더 스크린샷). 전체 테스트 159/159.
...
1. ~~**M1.3–M1.6** — 로그인 OK body 확정 → 서버 응답 완성 → 실클라 로그인 증거~~ ✅ **완료 2026-07-09** (M1.3 0x7001 20B 확정·구현, M1.4 서버 응답, M1.5 클라 루프백 패치, M1.6 로그인+로비 라이브 증거). 다음 임계경로는 아래 3.
2. ~~**M0.5** — content audit reviewQueue 상위 정리~~ ✅ **완료 2026-07-09** — 갤럭시 GREEN(85/79좌표), broken-ref 732건은 전부 리셋 전 잔재·서버 비소비로 수리 불필요 판정. (감사 JSON 재생성 커밋됨)
3. **M2 (현 임계경로)** — 캐릭터 작성/삭제/선택 실클라 검증. `inei00` 캐릭터 0개 상태에서 0x1008 생성 경로 → 월드 진입. 서버는 0x2009→0x200a create-pending까지 응답 확인됨.
