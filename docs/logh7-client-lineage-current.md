...
- 모든 단계가 끝난 뒤에만 `LOGH7-WINE-LINEAGE-V1` `client-lineage.json`을 작성하고 staging directory를 원자적으로 승격한다. 중간 실패 시 staging을 지우며 final root를 만들지 않는다.
...
성공 출력의 `manifestPath`와 `workingPath`를 Wine P0 preflight의 `--lineage-manifest`, `--client-exe`로 전달한다. 이 재구성 자체는 host-side 정적 패치 단계이며 Wine을 실행하지 않는다. 이후 실제 클라이언트 시작은 `logh7-wine-live-qa` 계약의 저장소 밖 전용 `WINEPREFIX`, exact Wine tool 절대경로, run evidence gate를 별도로 통과해야 한다.
...
- 이 gate는 계보 재현 증거다. Wine 창 렌더, 로그인, 월드 진입, 두 클라이언트 동기화 또는 run9 회귀 성공을 대신하지 않는다.
