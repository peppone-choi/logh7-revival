...
## Inputs
`.ai/task.md` 계약, 대상 환경(로컬 dev / compose / native Windows 또는 Wine 라이브 QA).
...
3. 라이브 QA(원본 클라이언트): `sys.platform`을 기록한다. `win32`는 Wine 없이 검증된 EXE를 직접 실행하고, `darwin`·`linux`는 저장소 밖 run 전용 `WINEPREFIX`, 명시적 `PREFIX_MODE=win32|wow64`, absolute Wine toolchain만 사용한다. 그 밖의 host는 unsupported로 차단한다. 모든 mode에서 EXE hash·image base·sentinel 불일치는 launch/attach/patch 전 fail-closed한다.
...
## Failure Handling
기동 실패 → `/debug`. 공통 lineage/evidence 또는 선택한 runtime의 environment gate 차단은 우회하지 말고 요구사항을 채운다 — fail-closed는 버그가 아니다.
...
실행 중인 프로세스·`RUNTIME_MODE`·runtime 자원(native session 또는 Wine prefix)·run 영수증 경로를 `.ai/handoff.md`에 기록한다.
