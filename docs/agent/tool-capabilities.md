...
- 스킬 부트스트랩: `scripts/agent/bootstrap-skills.sh` — canonical(`.agents/skills/`, skills.sh 프로젝트 스코프)을 기준으로 `.claude/skills/`의 MISSING/STALE을 `scripts/agent/required-skills.tsv` 매니페스트대로 점검(`--check`, 기본, fail-open)·동기화(`--sync`, MISSING만 복사, STALE은 `--force` 없이 건너뜀). Codex는 `.codex/agents/*.toml`이 canonical을 직접 참조해 물리 복사가 불필요하므로(실증) canonical 경로 실존만 확인 — `.codex/skills/`는 대상이 아님. SessionStart 훅(`--once`, 스탬프 `.omc/state/skills-bootstrap.stamp`)로 세션당 1회 자동 실행, `--strict`는 CI/수동 점검용 — 이 로컬 모드들은 네트워크·npx 호출이 전혀 없다. skills.sh 레지스트리 조회·검색은 별도 수동 모드(`--status`, `--search <query>`)로 분리되어 이때만 `npx skills`를 호출하며, 신규 외부 스킬 설치는 항상 안내만 하고 사람 승인 없는 자동 설치는 하지 않음
