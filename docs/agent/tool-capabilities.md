...
| CI/CD | ✅ (`.github/workflows/ci.yml`·`claude.yml`, `.coderabbit.yaml`) | — | ✅ **라이브** — Secret 등록·CodeRabbit App 설치 완료(2026-07-16), PR #6에서 CI 첫 런 녹색(41s·39s, 489 테스트). Claude GHA 리뷰는 main 병합 후 다음 PR부터(첫 PR 스킵은 공식 정상 동작), CodeRabbit 실리뷰는 50파일 이하 PR에서. Claude GHA review job은 org/repo 변수 ANTHROPIC_MODEL이 유효 모델에 1M 접미사(claude-opus-4-8[1m])를 붙여 is_error로 실패해 왔다. workflow model: pin(PR #173)만으론 불충분, step env: ANTHROPIC_MODEL 오버라이드로 해결(PR #175, review PASS·model claude-opus-4-8). CI test job·CodeRabbit은 정상 동작 | 첫 실리뷰 실측은 Phase 3 소형 PR |
| Jira | ✅ `.mcp.json`의 `atlassian` MCP — 사용자 브라우저 OAuth 승인 후 read/write 실증(2026-07-17, LOGH7-43/18 read-back) | ⚠️ Atlassian Rovo 커넥터는 사용 불가 — 허용 테넌트가 옛 `pepponechoi.atlassian.net`뿐이고 해당 테넌트는 suspended-inactivity 403 (2026-07-17 관측) | ✅ 사이트 `pepponechoi-jira.atlassian.net`(cloudId `300c260a-54a7-4ab5-b843-ae94bf68dcd6`), 프로젝트 **`LOGH7`**("은하영웅전설7 부활") — 분해 루틴 실증 완료. 옛 `pepponechoi` 사이트는 사용하지 않음 | 계획은 로컬 Markdown이 기본 폴백, Jira 분해 루틴은 `docs/agent/lifecycle-planning.md`. 익명 REST·미로그인 브라우저는 read-back에도 불가 |
...
| 2026-07-18 | 옵시디언 볼트 경로(이 Windows 머신) | 이 머신 클론은 `E:\obsidian-tech-vault`, LOGH7 노트는 `E:\obsidian-tech-vault\1. 프로젝트\은하영웅전설 7 리바이벌\`(현재 상태·로드맵·커밋 로그·리버스 엔지니어링·서버·클라이언트 + 신규 사회 시뮬레이션 노트). 사용자 지시(2026-07-18)로 확인. **잔여**: `LOGH7_VAULT_DIR`이 이 셸/훅 env에 미설정 — 문서 게이트 자동 볼트 검사가 활성화되려면 `.claude/settings.local.json`(env, gitignored)에 `LOGH7_VAULT_DIR=E:\obsidian-tech-vault` 설정 필요(사람) | 현재는 볼트 갱신을 수동/명시 수행. 노트 편집만(사용자가 obsidian 동기화), commit·push 안 함 |
| 2026-07-21 | 운영 규칙 2건 추가 (Wave-1 전달 교훈) | CLAUDE.md `Never`·AGENTS.md `Git Safety`: (1) 파일 내용 해시는 LF 정규화 후 계산 — Windows 로컬(CRLF) vs CI Linux(LF) 불일치로 로컬만 통과하는 숨은 결함 방지; (2) 서브에이전트 브리프에서 작업트리 되돌리는 git(`reset`·`checkout --`·`restore`·`stash`·`clean`)·blanket `git add -A` 금지, "보호"≠원상복구(`.codex/config.toml` 미커밋 변경 파괴 사고) | PR #239(`9d8f3565`), 사용자 승인 |
