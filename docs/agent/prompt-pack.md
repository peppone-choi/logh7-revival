...
| 커맨드 (`.claude/commands/`) | 팩 | 구분 |
|---|---|---|
| `/start-task` | 계획/기획 팩 | 신규 |
| `/analyze` | 기능 분석 | 기존 |
| `/implement` | 기능 구현 | 기존 |
| `/debug` | 근본 원인 디버깅 | 기존 |
| `/review` | 코드 리뷰 | 기존 |
| `/verify` | 검증 팩 | 신규 |
| `/checkpoint` | 작업 인수인계 | 기존 |

도메인 팩(커맨드에 1:1 매핑되지 않음 — 담당 에이전트 대상, `.claude/agents/`):
...
### Required Context
`.codex/skills/logh7-wine-live-qa/SKILL.md`(필수 입력·fail-closed 조건), `docs/logh7-client-lineage-current.md`(EXE hash·sentinel·manifest), `.ai/known-issues.md`의 게이트 차단 현황.
### Output Format
run evidence manifest(REPO_ROOT/WINE_BIN/WINEBOOT_BIN/WINESERVER_BIN/WINEPREFIX/RUN_ID/RUN_MODE 등) / 스크린샷·서버로그 경로 / 서버 로그 ↔ 클라 화면 교차비교 결과 / 발견된 버그의 담당 에이전트 라우팅.
### Constraints
기본 `~/.wine` 사용 금지 — 저장소 밖 전용·run 전용 `WINEPREFIX`만 사용한다. EXE hash·image base·sentinel이 lineage manifest와 불일치하는 상태로 launch/attach/patch 금지(fail-closed는 버그가 아니다). `node.exe` 블랭킷 kill 금지 — 검증된 PID만 종료. 스크린샷·서버로그 증거 없이 완료 주장 금지.
### Stop Conditions
`WINE_BIN`/`WINEBOOT_BIN`/`WINESERVER_BIN`/`WINEPREFIX` 중 하나라도 절대경로가 아니거나 해석 불가, 또는 EXE hash/sentinel 불일치면 시작하지 말고 `blocked` manifest를 남기고 종료. 게이트가 이미 차단 상태(`.ai/known-issues.md`)면 우회 시도 없이 "라이브 미검증"으로 명시 보고.
### Template
```
[라이브 QA] <검증 시나리오(로그인/로비/월드/채팅)>
.codex/skills/logh7-wine-live-qa/SKILL.md의 필수 입력(REPO_ROOT/WINE_BIN/WINEBOOT_BIN/
WINESERVER_BIN/WINEPREFIX/RUN_ID/RUN_MODE/CLIENT_EXE/LINEAGE_MANIFEST)을 모두 채우고
fail-closed 조건(상대경로·기본 ~/.wine·EXE hash/sentinel 불일치)을 먼저 확인하라.
