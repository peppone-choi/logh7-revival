...
> **2026-07-16 계보 정정:** 이 문서가 `2848be76…`을 프리스틴 설치본이라고 부르고
> `127.0.0.1`이 컴파일된 기본값이라고 결론 낸 부분은 실바이트 재구성으로 반증됐다.
> CD/설치 원본 `bd19263c…`에는 raw `0x36ee3c`에 `202.8.80.179\0`이 있고,
> 동일 13바이트 슬롯을 `127.0.0.1`+NUL padding으로 바꾸면 정확히 `2848be76…`이 된다.
> 현재 실행 계약과 전체 체인은 `docs/logh7-client-lineage-current.md` 및
> `server/content/client/logh7-loopback-client-patch.json`을 따른다. 아래 본문은
> 2026-07-11 당시 감사 기록을 보존한 역사 자료이며 current provenance로 인용하지 않는다.
