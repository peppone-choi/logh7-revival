...
- 0x0410 CommandEvacuateTroops — CLOSED (2026-07-02, CONTENT-05 후속): processCommand 브랜치 추가(`server/src/server/logh7-command-engine.mjs`, parseInboundSortie 재사용, 새 `evacuateTroop` 상태변이 `logh7-world-state.mjs`). 신규 와이어 값은 생산하지 않음 — 이미 P0 확정된 NotifyMovedTroop(0x429)만 재사용. 클라이언트가 이 opcode를 실제로 보내는 소비측 FUN_ 경로는 여전히 P0 미재확인이라 이 처리 자체는 P2로 유지.
...
- **0x0410** CommandEvacuateTroops [C->S] server:CLOSED (2026-07-02) — `server/src/server/logh7-command-engine.mjs` processCommand COMMAND_EVACUATE_TROOPS_CODE branch (parseInboundSortie reuse + `state.evacuateTroop`, no new wire values, P2 pending client-side FUN_ re-confirmation)
