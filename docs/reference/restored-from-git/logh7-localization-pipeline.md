...
`build-installed`는 기본으로 `.omo/work/logh7-ko-overlay/exe/G7MTClient.playable.exe`와
그 `.playable-manifest.json`을 검증한다. 다른 클라이언트를 쓰는 재현 실험은
`--playable-client <exe>`를 명시하되, 옆 manifest의 `stack`에 `menufix`, `dlgfix`,
`earlygrid-ringclear`가 모두 있어야 한다.
