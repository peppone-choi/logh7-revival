...
- native Windows에서 Wine 입력·명령 요구, Wine mode에서 기본 `~/.wine` 사용, EXE hash·image base·sentinel 불일치 상태로 launch/attach/patch — fail-closed는 버그가 아니다
- 다른 에이전트 소유 파일 수정 (`.ai/ownership.md`)
- 파일 내용 해시를 라인엔딩 정규화(LF) 없이 계산·비교 — 로컬 Windows(CRLF)와 CI Linux(LF, 원시 blob) 불일치로 로컬만 통과하고 CI만 실패하는 숨은 결함. 파일 해시는 LF 정규화(`canonicalLf`) 후 계산하고, 라인엔딩·바이트 민감 코드는 LF 기준(CI 동일)으로 검증한다
- 서브에이전트 브리프에서 작업트리를 되돌리는 git(`reset`·`checkout -- <path>`·`restore`·`stash`·`clean`)이나 blanket `git add -A`/`git add .` 지시 — "보호"는 원상복구가 아니다; 미커밋 사용자 자산(예: `.codex/config.toml`)을 파괴한다. 스테이징은 명시 경로만 쓰고, 위임 후 보호 파일 상태를 재확인한다
...
**문서 현행화 게이트(Stop 훅 강제)**: 파일을 변경한 턴은 관련 `docs/` 현행 문서, 루트 `AGENTS.md`·`CLAUDE.md`, `.ai/current-state.md`, (`LOGH7_VAULT_DIR` 설정 머신에서는) 옵시디언 볼트까지 갱신해야 끝난다. 반영할 것이 없으면 그 근거를 보고에 명시한다. 볼트 정본은 `peppone-choi/obsidian-tech-vault` 클론 — 머신별 경로 설정은 `docs/agent/tool-capabilities.md` 하네스 이력 참조.

## 하네스

- logh7 도메인 작업(자산추출·RE·프로토콜·서버·한글화·라이브QA)은 `logh7-orchestrator` 스킬로 분해. 단순 질문은 직접 응답.
- 라이브QA는 `.agents/skills/logh7-wine-live-qa/SKILL.md`가 정본이다. Claude의 `live-qa`는 native Windows에서 Wine 없이 직접 실행하고 macOS/Linux에서만 격리 Wine을 사용하며, 그 외 host는 차단한다.
- 에이전트 팀: `.claude/agents/` 6종 (frontmatter model 없음 — 호출 시점에 지정).
- 설치 스택·도구 실사·하네스 변경 이력: `docs/agent/tool-capabilities.md`.
- 스킬 부트스트랩: `scripts/agent/bootstrap-skills.sh --check`(SessionStart 훅이 `--once` 자동 실행, 매니페스트 `scripts/agent/required-skills.tsv`). canonical은 `.agents/skills/`(skills.sh 표준) — 신규 외부 스킬 설치는 사람 승인 후 `npx skills add`.
- 커맨드 7종은 `docs/agent/prompt-pack.md`의 대응 팩(7섹션)을 명시 로드한다 — 매핑 표는 그 문서 앞머리.
- MCP 정의는 `.mcp.json`(커밋), 활성화는 `.claude/settings.local.json` 명시 allowlist — Jira 기획 루틴은 `docs/agent/lifecycle-planning.md` 참조.
