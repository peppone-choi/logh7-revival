...
2026-07-05 G076 로그인 확인 버튼 좌표 재교정 3회 동일증상(클릭→ID 콤보 NO DATA 드롭다운 재오픈, 서버 로그인 시도 0건)으로 Blocked-Loop Rule 적용해 피벗. 좌표 추측 대신 DPI/스케일링 검증 또는 히트테스트 코드 정적 RE가 다음 전략. 상세는 루트 `CLAUDE.md` 동일 날짜 항목.

2026-07-05 G075 cp932 실주행 QA 중 C002 근본원인 2건 라이브 확정+수정: `SetForegroundWindow` 백그라운드 프로세스 no-op 문제를 `AttachThreadInput` 우회 공용 헬퍼(`_force_foreground`)로 6개 호출부 전부 교체, ID 필드 재시작 간 텍스트 누적 문제를 `_clear_field`(하드웨어 백스페이스)로 수정. RE 32/32 통과. 로그인 제출 좌표 `(352,347)`가 실제로는 제출 버튼이 아니라 ID 콤보 재오픈 좌표였음이 드러나 진짜 확인 버튼 좌표 재교정이 남음 — cp932 실채팅 검증은 이월. 상세는 루트 `CLAUDE.md` 동일 날짜 항목.
