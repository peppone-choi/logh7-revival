...
const COMMON = [
  'LOGH VII revival root is C:\\Users\\by0ng\\OneDrive\\Desktop\\logh7-revival. 권위적 Node.js 서버(server/src/server/*.mjs)가 디컴파일 G7MTClient.exe가 파싱하는 와이어를 emit.',
  'RE 조회: `cd RE && python -m tools.logh7_redex func 0x<addr>` / `python -m tools.logh7_redex grep "<sym>"`. RE 인덱스 RE/.omo/ghidra/export/G7MTClient/.',
...
const domains = [
  { key: 'inworld-command', label: '갭:인월드명령/함대배치', prompt:
    ...
    + '갭 발굴: (a) 0x0b01을 내는 전략 move-UI 제스처/모드(FUN_004d6310 호출 조건, HUD 이동버튼)를 RE로 미확정, (b) 진영별 카논 시작 함대 배치 셀이 서버에 없음(매뉴얼 server/content/manual/* + gin7manualsaved.pdf에서 오딘/하이네센 등 시작 배치 추출 필요), '
    + '(c) cave가 하드코딩 2550 대신 실제 own-fleet 셀(--cell-mem, own char-id@+0x3584a0→+0x36a8b4→flagship+0x24→0x41a368+0x08) 미사용, (d) in-world 함대조작(0x0400 이동/0x0423·0x0424 broadcast) 라이브 미검증. server/src/server/logh7-login-session.mjs, logh7-command-engine.mjs 참조.' },
  ...
  { key: 'content-impl', label: '갭:컨텐츠구현', prompt:
    ...
    + 'server/src/server/logh7-imperial-titles.mjs, logh7-personnel.mjs, logh7-base-economy.mjs, docs/logh7-post-permissions.md 참조.' },
  { key: 'ui-values', label: '갭:UI값표시', prompt:
    '도메인=UI 값 표시(HUD/패널이 실제 수치를 제대로 보여주기). 갭: (a) 0x031f 경제레코드 다수 필드가 PROVISIONAL 오프셋(라벨 미해결)이라 population/food/approval 등이 0 유지→패널 빈값(server/src/server/logh7-base-record.mjs:70-78). '
  ...
  { key: 'remaster', label: '갭:리마스터/해상도', prompt:
    ...
    + 'RE/tools/logh7_graphics_config.py, logh7_texture_pipeline.py, docs/logh7-graphics-remaster.md 참조.' },
  { key: 'localization', label: '갭:현지화/텍스트', prompt:
    ...
    + '(c) 동적 생성 문자열(날짜·수치 포맷)·신규 컨텐츠(작위명 등) 번역 누락, (d) 일↔한 텍스트만 교체 파이프라인 일반화. RE/tools/logh7_binary_strings.py, server/content/localization/* 참조.' },
  { key: 'modding', label: '갭:모딩/확장', prompt:
    ...
    + '(b) 클라측 데이터(String.txt/model/galaxy) 외부화 모딩 미흡, (c) 커스텀 국가/시나리오 데이터주도 미완, (d) 모드 문서/스키마. server/src/server/logh7-mod-loader.mjs, RE/tools/logh7_tcf_pack.py 참조.' },
  { key: 'server-correctness', label: '갭:서버데이터정합', prompt:
    ...
    + 'opcode 충돌(0x0337), 미구현 핸들러 분기. server/src/server/*.mjs 전반 + docs/logh7-info-records-wire.md, logh7-implementation-specs.md 대조로 "RE로 닫아야 할" 항목 발굴.' },
]
