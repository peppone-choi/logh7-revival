...
const STATE_INTRO = `
LOGH VII revival 저장소(E:\\logh7-revival). 먼저 AGENTS.md, docs/logh7-current-work-register-2026-06-17.md,
...
위 recommendedChange를 최소 범위로 구현한다. src/server/*.mjs는 ESM이다. 관련 tests/server/*.test.mjs도 같이 갱신/추가한다.
...
\`npm run test:server\` 또는 영향받은 \`node --test tests/server/<관련>.test.mjs\`를 실행하고 결과를 TEST로 반환한다.
...
return {
  ...
  reports,
  note: '메인 에이전트는 이 결과를 검토해 docs/logh7-loop-state.md에 증거/다음 항목을 갱신하고, 실클라 표면이 필요한 항목은 tools/logh7_ui_explorer.py로 닫는다.',
}
