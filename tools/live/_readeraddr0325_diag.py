#!/usr/bin/env python3
"""실 월드리더 주소 확정 (무변조). FUN_00612510 onLeave → *(retval)+8 = 리더주소.
0x0323 vs 0x0325 리더 동일함수인지 + OnRecv/디스패치 대조. usage: py -3 _readeraddr0325_diag.py <evdir>
"""
...
PROBE_JS = Path(__file__).resolve().parent / "_frida_readeraddr0325_probe.js"
...
def main() -> int:
    ...
    def fin(rc):
        (evdir / "readeraddr.jsonl").write_text("\n".join(json.dumps(e, ensure_ascii=False) for e in events) + "\n", encoding="utf-8")
        try: srv.terminate()
    ...
    def on_message(m, _d):
        ...
        if ev == "reader_addr":
            note(f"  === READER_ADDR code={p.get('code')} reader={p.get('reader')} vtbl={p.get('vtbl')} handler={p.get('handler')} ===")
        elif ev == "reader_addr_err":
            note(f"  reader_addr_err code={p.get('code')} {p.get('err')}")
        elif ev == "onrecv":
    ...
    while time.time() - t0 < 20:
        ...
        time.sleep(0.5)
    try: note(f"readers rpc={script.exports_sync.readers()}")
    except Exception as e: note(f"rpc fail {e}")
    if alive(): proc.terminate()

    # 요약
    ra = {e.get("code"): e.get("reader") for e in events if e.get("ev") == "reader_addr"}
    onr = [e.get("code") for e in events if e.get("ev") == "onrecv"]
    disp = [e.get("code") for e in events if e.get("ev") == "disp"]
    note("===== 실 리더 주소 =====")
    note(f"코드별 리더주소: {ra}")
    note(f"0x0325 리더 = {ra.get('0x325')}  vs  0x0323 리더 = {ra.get('0x323')}  동일={ra.get('0x325')==ra.get('0x323') if ra.get('0x325') and ra.get('0x323') else 'N/A'}")
