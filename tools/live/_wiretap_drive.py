#!/usr/bin/env python3
"""인바운드 와이어 탭 드라이버 (진단 전용, 서버/클라 무변조).

목표: 서버가 바이트 정확히 write한 0x0315 응답(~5016B)이 클라 소켓에 도착하나?
      도착하면 클라 디스패처가 0x315를 디스패치하나? 한 런에서 교차 측정.
프로브: _frida_wiretap_probe.js (ws2_32 recv/WSARecv 재조립 + FUN_004ba2b0 상관).

usage: python _wiretap_drive.py <evidence-dir>
"""
...
PROBE_JS = Path(__file__).resolve().parent / "_frida_wiretap_probe.js"
...
def main() -> int:
    ...
    def on_message(message, _data):
        if message["type"] == "send":
            p = message["payload"]
            events.append(p)
            if isinstance(p, dict) and p.get("ev") == "frame":
                note(f"  FRAME i={p.get('i')} total={p.get('total')} "
                     f"code={p.get('code')} hdr={p.get('hdr')}")
            if isinstance(p, dict) and p.get("ev") == "decrypt-fail":
        ...
        try:
            ...
            if rdy:
                note(f"probe loaded (try {attempt}) base={rdy[0].get('base')} "
                     f"recv={rdy[0].get('recv')} wsarecv={rdy[0].get('wsarecv')}")
                break
    ...
    def pull(tag):
        ...
        note(f"[{tag}] recvCalls={s['recvCalls']} inbound={s['totalInbound']} "
             ...
             f"c315={s['count315']} c307={s['count307']} c305={s['count305']}")
        note(f"[{tag}] bigFrames={s['bigFrames']}")
        note(f"[{tag}] dispWorkSeq={s['dispWorkSeq']}")
        note(f"[{tag}] enqWorkSeq={s.get('enqWorkSeq')} failCount={s.get('failCount')}")
        if s.get("desyncSockets"):
            note(f"[{tag}] DESYNC sockets={s['desyncSockets']} pending={s['pendingBytes']}")
    ...
    note("=== monitor world-load (55s), poll wiretap + dispatch ===")
    t0 = time.time()
    si = 5
    last = 0
    ...
    note("=== VERDICT ===")
    if final:
        bigs = final.get("bigFrames", [])
        note(f"big inbound frames (>=3000B), in order: {bigs}")
        # 0x0307 응답(대형, ~58KB) 이후 ~5KB(0x0315) 프레임이 왔나?
        big_after_max = None
        maxsize = max((b["total"] for b in bigs), default=0)
        mid = [b for b in bigs if 3000 <= b["total"] <= 12000]
        note(f"largest frame={maxsize}B; mid-size(3-12KB) frames={mid}")
        if final["count315"] > 0:
            note("VERDICT-A2: 0x315 DISPATCHED (frame arrived AND dispatched). desync 아님.")
        elif mid:
            note("VERDICT-B: ~5KB frame ARRIVED at socket but 0x315 NOT dispatched "
                 "-> client transport parser DROP. FUN_004b8850/decrypt/checksum/id gate 조사 필요.")
        else:
            note("VERDICT-A1: NO ~5KB(0x0315) frame arrived at socket after big frame "
                 "-> server write/socket-layer 문제. server trace 어느 커넥션에 write했나 대조.")
        note(f"final dispWorkSeq={final.get('dispWorkSeq')}")
        note(f"final enqWorkSeq={final.get('enqWorkSeq')} (0x315 여기 없으면 enqueue 전 드롭)")
        note(f"decrypt-fail count={final.get('failCount')}")
        for ff in final.get("failFrames", []):
            note(f"  FAIL[{ff['n']}] envCode={ff['envCode']} hex={ff['hex']}")

    (evdir / "frida-events.jsonl").write_text(
        "\n".join(json.dumps(e, ensure_ascii=False) for e in events) + "\n", encoding="utf-8")
    note(f"wrote {len(events)} events, pid={pid.value} (kept alive for repull)")
