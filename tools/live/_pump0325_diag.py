#!/usr/bin/env python3
"""0x0325 코덱-소비 펌프 탈락점 계측 (무변조).
FUN_006103e0(attach) + FUN_00612510(lookup) 훅으로 0x0325가 룩업에서 null(드롭)인지,
형제(0x0323)는 non-null인지 대조. usage: py -3 _pump0325_diag.py <evdir>
"""
...
PROBE_JS = Path(__file__).resolve().parent / "_frida_pump0325_probe.js"
...
def main() -> int:
    ...
    def on_message(m, _d):
        ...
        if ev == "attach":
            tag = "  <<< ATTACH" if p.get("code") == "0x325" else "  attach"
            note(f"{tag} code={p.get('code')} len={p.get('len')} outbuf={p.get('outbuf')} sumHi={p.get('sumHi')}")
        elif ev == "lookup":
            tag = "  <<< LOOKUP" if p.get("code") == "0x325" else "  lookup"
            note(f"{tag} code={p.get('code')} arg={p.get('arg')} ret={p.get('ret')} isNull={p.get('isNull')}")
        elif ev == "disp":
            ...
            note(f"  probe ready base={p.get('base')} attach={p.get('attach')} lookup={p.get('lookup')}")
    ...
    # 요약: 0x0325 룩업 이분
    att = [e for e in events if e.get("ev") == "attach"]
    lk = [e for e in events if e.get("ev") == "lookup"]
    att325 = [e for e in att if e.get("code") == "0x325"]
    lk325 = [e for e in lk if e.get("code") == "0x325"]
    lk323 = [e for e in lk if e.get("code") == "0x323"]
    disp_codes = [e.get("code") for e in events if e.get("ev") == "disp"]
    note("===== 0x0325 펌프 룩업 이분 =====")
    note(f"[attach] 0x325 도달: {len(att325)}건  len={[e.get('len') for e in att325]} sumHi={[e.get('sumHi') for e in att325]}")
    note(f"[lookup] 0x325: {[(e.get('arg'), e.get('ret'), e.get('isNull')) for e in lk325]}")
    note(f"[lookup] 0x323(대조): {[(e.get('arg'), e.get('ret'), e.get('isNull')) for e in lk323]}")
    note(f"[디스패치] 0x325: {'O' if '0x325' in disp_codes else 'X'}")
    # 판정
    if att325 and lk325 and all(e.get("isNull") for e in lk325):
        note("판정→ 룩업 null 드롭 확정: 0x325가 FUN_00612510에서 null 반환 → L295253 드롭. arg 상위16(sumHi) 대조로 64KB경계 가설 확인")
    elif att325 and not lk325:
        note("판정→ attach 도달하나 lookup 미도달: attach~lookup 사이 다른 조기탈출")
    elif not att325:
        note("판정→ attach 미도달: 코덱 반환이 펌프서 null 취급(재검토 필요)")
    return fin(0)
