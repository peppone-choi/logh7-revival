#!/usr/bin/env python3
"""0x0325 리더 attach_len 실값 이분 계측 (무변조).
FUN_006126b0 리더 진입 시 [arg0+8](attach_len)을 0x0325 vs 0x0323 대조. 52810이면 리더 무혐의,
작은값/wrap이면 서버 프레이밍 길이 오류. usage: py -3 _readerlen0325_diag.py <evdir>
"""
...
PROBE_JS = Path(__file__).resolve().parent / "_frida_readerlen0325_probe.js"
...
def main() -> int:
    ...
    def fin(rc):
        (evdir / "readerlen.jsonl").write_text("\n".join(json.dumps(e, ensure_ascii=False) for e in events) + "\n", encoding="utf-8")
        try: srv.terminate()
    ...
    def on_message(m, _d):
        ...
        if ev == "reader":
            ...
            note(f"{tag} code={p.get('code')} attach_len={p.get('attach_len')}({p.get('attach_len_hex')}) "
                 f"from006103e0={p.get('attach_from_006103e0')} buf={p.get('bufptr')}")
        elif ev == "onrecv":
            ...
            note(f"  probe ready base={p.get('base')} reader={p.get('reader')}")
    ...
    # 요약: 0x0325 리더 attach_len 이분
    rd = [e for e in events if e.get("ev") == "reader"]
    rd325 = [e for e in rd if e.get("code") == "0x325"]
    rd323 = [e for e in rd if e.get("code") == "0x323"]
    onr = [e.get("code") for e in events if e.get("ev") == "onrecv"]
    disp = [e.get("code") for e in events if e.get("ev") == "disp"]
    note("===== 0x0325 리더 attach_len 이분 =====")
    note(f"[리더 진입] 0x325: {len(rd325)}건  ★진입={'O' if rd325 else 'X'}")
    note(f"[attach_len] 0x325: {[(e.get('attach_len'), e.get('attach_len_hex'), 'from6103e0='+str(e.get('attach_from_006103e0'))) for e in rd325]}")
    note(f"[attach_len] 0x323(대조): {[(e.get('attach_len'), e.get('attach_len_hex')) for e in rd323]}")
    note(f"[OnRecv] 0x325: {'O' if '0x325' in onr else 'X'}  [디스패치] 0x325: {'O' if '0x325' in disp else 'X'}")
    # 판정
    if not rd325:
        note("판정→ 리더 미진입: 0x325가 FUN_006126b0 진입 안 함 → 리더 이전(vcall 라우팅) 소실")
    elif rd325 and all(e.get("attach_len") == 52810 for e in rd325):
        note("판정→ 리더 무혐의(attach_len=52810 정상): 소실은 리더 이후 단계(추가 조사)")
    elif rd325:
        note(f"판정→ ★서버 프레이밍 길이 오류: attach_len이 52810 아님 → {[e.get('attach_len') for e in rd325]} (역산 필요)")
    return fin(0)
