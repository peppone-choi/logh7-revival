...
def click(sx,sy):
    ...
    user32.SendInput(1,ctypes.byref(i),ctypes.sizeof(INPUT)); time.sleep(0.04)
    ...
    user32.SendInput(1,ctypes.byref(u),ctypes.sizeof(INPUT)); time.sleep(0.15)
def type_u(text):
    for ch in text:
...
grab(hwnd).save(sd/'e01-login.png')
LOGIN_REF=(644,484)
# ID
cx,cy=scale(LOGIN_REF,(374,290),cw,ch)
user32.SetForegroundWindow(hwnd); click(ox+cx,oy+cy); time.sleep(0.1)
for _ in range(14):
    type_u(chr(8))
type_u('inei00'); time.sleep(0.15)
# PW
cx,cy=scale(LOGIN_REF,(376,318),cw,ch)
user32.SetForegroundWindow(hwnd); click(ox+cx,oy+cy); time.sleep(0.1)
for _ in range(14):
    type_u(chr(8))
type_u('dummy'); time.sleep(0.15)
# LOGIN
cx,cy=scale(LOGIN_REF,(352,347),cw,ch)
user32.SetForegroundWindow(hwnd); click(ox+cx,oy+cy)
print('login clicked')
...
for i in range(25):
    time.sleep(0.5)
    if has_crash():
        print('crash', i); grab(hwnd).save(sd/'e02-crash.png'); break
    if not user32.IsWindow(hwnd):
        print('dead', i); break
    ox,oy,cw,ch=geom(hwnd); img=grab(hwnd); sc=menu_blue(img)
    print(f'wait{i} {cw}x{ch} blue={sc:.3f}')
    if cw>=1000 and sc>0.02:
        img.save(sd/'e02-lobby.png'); print('LOBBY'); lobby=True; break
if not lobby:
    if user32.IsWindow(hwnd): grab(hwnd).save(sd/'e02-fail.png')
    print('no lobby'); sys.exit(1)

# stability 4s
for i in range(8):
    time.sleep(0.5)
    if has_crash() or not user32.IsWindow(hwnd):
        print('unstable', i, 'crash', has_crash()); 
        if user32.IsWindow(hwnd): grab(hwnd).save(sd/'e03-unstable.png')
        sys.exit(2)
print('STABLE')
grab(hwnd).save(sd/'e03-stable.png')

ox,oy,cw,ch=geom(hwnd)
for i,(cx,cy) in enumerate([(90,212),(100,205),(100,220)]):
    user32.SetForegroundWindow(hwnd); click(ox+cx,oy+cy); time.sleep(1.4)
    if has_crash():
        print('crash create'); break
    img=grab(hwnd); fs=face_score(img)
    img.save(sd/f'e04-try{i}.png')
    print(f'try{i} ({cx},{cy}) face={fs:.4f}')
    if fs>0.008:
        print('ORIGIN')
        user32.SetForegroundWindow(hwnd); click(ox+515,oy+310); time.sleep(0.8)
        grab(hwnd).save(sd/'e05-face.png')
        user32.SetForegroundWindow(hwnd); click(ox+655,oy+585); time.sleep(1.5)
        img2=grab(hwnd); img2.save(sd/'e06-ok.png')
        print('ok face', face_score(img2), 'crash', has_crash())
        break
print('done')
for ln in Path('server/data/live-manual-trace.jsonl').read_text(encoding='utf-8',errors='ignore').strip().splitlines()[-18:]:
    print(ln[:300])
