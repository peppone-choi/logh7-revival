"""Runtime probe: record every inbound inner code that reaches the LOBBY app dispatcher.

Hooks the lobby message dispatcher head at 0x004bd7d4 (`cmp eax, 0x2000`), where EAX
holds the inner code (0x2000/0x2001/0x2003/.../0x200b/0x7001) of every server->client
lobby message the transport router has ALREADY accepted and routed to the application
layer (consumer table at 0x4be324; the 0x2001 case 0x4bdb70 sets the lobby success flag
*(0x7ccffc)+0x35837b that the FSM polls).

Decisive question this answers (handoff G167): when conn2 closes right after our 0x2001
reply, does our 0x2001 REACH this dispatcher (=> routed fine; the close is a downstream
FSM/teardown issue) or NOT (=> it was dropped earlier at decipher_message 0x645db0 or the
transport router's handler-map lookup 0x612510 -> drop/close 0x612378). If the ring shows
no 0x2001 while the client tears conn2 down, the blocker is the transport router (the
G096-G101 "empty/missing receive-handler map" — but for the lobby connection).

Hook is non-destructive: 5-byte jmp to a code cave; the trampoline records EAX/EBX/ESI of
the saved register frame, replays the original `cmp eax,0x2000`, and jumps back to 0x4bd7d9.

Saved-frame layout after `pushfd; pushad`: ESP-> EDI,ESI,EBP,ESP,EBX,EDX,ECX,EAX,EFLAGS,
so EAX@[esp+0x1c], EBX@[esp+0x10], ESI@[esp+0x04].

Ring record (32-byte stride): [magic 4][callIndex 4][eax(innerCode) 4][ebx 4][esi 4][pad 12].
...
HOOK_VA: Final[int] = 0x004BD7D4
CONTINUATION_VA: Final[int] = 0x004BD7D9
HOOK_LENGTH: Final[int] = 5
ORIGINAL_HEX: Final[str] = "3d00200000"  # cmp eax, 0x2000

BUFFER_OFFSET: Final[int] = 160  # trampoline lives below this; ring starts here (cave is 811B)
RECORD_BYTES: Final[int] = 32
RECORD_CAPACITY: Final[int] = 16
MAGIC: Final[bytes] = b"L7DP"  # LOGH7 Dispatch Probe


@dataclass(frozen=True, slots=True)
class LobbyDispatchProbePatch:
...
def apply_lobby_dispatch_probe_patch(source: Path, out: Path, manifest_out: Path) -> LobbyDispatchProbePatch:
    ...
    if original.hex() != ORIGINAL_HEX:
        raise ValueError(f"lobby dispatcher entry bytes drift at 0x{HOOK_VA:08x}: {original.hex()}")
    ...
    if len(trampoline) > cave.length_bytes:
        raise ValueError("lobby dispatch probe exceeds code cave capacity")
    ...
    patch = LobbyDispatchProbePatch(cave, hook_bytes.hex(), before, after)
...
def _build_trampoline(base_va: int, buffer_va: int) -> bytes:
    ...
    builder.append(bytes.fromhex(ORIGINAL_HEX))  # replay cmp eax, 0x2000
    builder.jmp_rel32(CONTINUATION_VA)
    if len(builder.data) > BUFFER_OFFSET:
        raise ValueError("lobby dispatch trampoline overlaps its ring buffer")
...
def _append_record(builder: X86Builder, buffer_va: int) -> None:
    ...
    # saved EAX (inner code) = [esp+0x1c] -> [edi+8]
    builder.append(b"\x8b\x44\x24\x1c\x89\x47\x08")
    # saved EBX = [esp+0x10] -> [edi+0x0c]
    builder.append(b"\x8b\x44\x24\x10\x89\x47\x0c")
    # saved ESI = [esp+0x04] -> [edi+0x10]
...
def decode_lobby_dispatch_ring(path: Path) -> dict[str, object]:
    data = path.read_bytes()
    if len(data) < 8:
        raise ValueError("lobby dispatch ring is too small")
    ...
    while offset + RECORD_BYTES <= len(data) and index < RECORD_CAPACITY:
        ...
        if chunk[:4] != MAGIC:
            ...
            call_index, inner_code, ebx, esi = struct.unpack_from("<IIII", chunk, 4)
            records.append(
                {
                    "index": index,
                    "callIndex": call_index,
                    "innerCodeHex": f"0x{inner_code & 0xffff:04x}",
                    "innerCodeRaw": inner_code,
                    "ebxHex": f"0x{ebx:08x}",
                    "esiHex": f"0x{esi:08x}",
                }
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch/decode the LOGH VII lobby dispatch-trace ring probe.")
    ...
    if args.command == "patch":
        patch_result = apply_lobby_dispatch_probe_patch(args.source, args.out, args.manifest_out)
        print(json.dumps(patch_result.to_json(), ensure_ascii=False, indent=2))
        return 0
    decoded = decode_lobby_dispatch_ring(args.ring)
