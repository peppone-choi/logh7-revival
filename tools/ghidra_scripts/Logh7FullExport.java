...
public class Logh7FullExport extends GhidraScript {
    ...
    // Force-create functions at vtable / pointer-table targets that auto-analysis left undefined
    // (decipher_message etc. are reached only via indirect calls), so the export is complete.
    private void createFunctionsFromPointers() throws Exception {
        ...
        while (di.hasNext() && !monitor.isCancelled()) {
            ...
            if (new CreateFunctionCmd(t).applyTo(currentProgram, monitor)) created++;
        }
        println("pointer-target functions: created=" + created + " execPtrs=" + seen);
    }
