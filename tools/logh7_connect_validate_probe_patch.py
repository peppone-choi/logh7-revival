"""Runtime probe: capture the transport connect-completion SOCKET-VALIDATION result.

Hooks 0x006154e3 (`add esp,4; test al,al`) right after the validation call 0x615d40
(= ioctlsocket(sock, FIONBIO 0x8004667e, &1); returns al = (result != -1)). On entry here
EAX/AL = the validation result, ESI = the transport `this`. If al!=0 the worker KEEPS
[transport+0x78]=2 (CONNECTED -> receive pump armed); if al==0 it reverts [+0x78]=0 at 0x6154ea
(receive never arms).

Decisive question (handoff G174): the connect-completion worker 0x615460 runs for BOTH
transports (entry [+0x78]=1). conn1 receives, conn2 does not. Does conn2's transport FAIL this
validation (al==0 -> reverted, receive never arms), or PASS (al!=0 -> then the receive-gate is
elsewhere)? This probe records, per call, the transport `this`, the validation result al, and
the handle [this+0x80] so conn1 vs conn2 can be told apart (and correlated with the connect
worker probe). callIndex order ~ connection order (conn1 first).

Displaces 5 bytes (add esp,4; test al,al) and replays them before 0x6154e8 (the jne).
Saved-frame layout after `pushfd; pushad`: EAX@[esp+0x1c], ESI@[esp+0x04].
Ring record (32-byte stride): [magic 4][callIndex 4][this(esi) 4][validResult(eax) 4][handle(+0x80) 4][pad 12].
...
HOOK_VA: Final[int] = 0x006154E3
CONTINUATION_VA: Final[int] = 0x006154E8
...
MAGIC: Final[bytes] = b"L7CV"  # LOGH7 Connect Validation


@dataclass(frozen=True, slots=True)
class ConnectValidateProbePatch:
...
def apply_connect_validate_probe_patch(source: Path, out: Path, manifest_out: Path) -> ConnectValidateProbePatch:
    ...
    if original.hex() != ORIGINAL_HEX:
        raise ValueError(f"connect-validation bytes drift at 0x{HOOK_VA:08x}: {original.hex()}")
    ...
    if len(trampoline) > cave.length_bytes:
        raise ValueError("connect-validation probe exceeds code cave capacity")
    ...
    patch = ConnectValidateProbePatch(cave, hook_bytes.hex(), before, after)
...
def _build_trampoline(base_va: int, buffer_va: int) -> bytes:
    ...
    if len(builder.data) > BUFFER_OFFSET:
        raise ValueError("connect-validation trampoline overlaps its ring buffer")
...
def _append_record(builder: X86Builder, buffer_va: int) -> None:
    ...
    # this (saved ESI) = [esp+0x04] -> ecx, and -> [edi+8]
    builder.append(b"\x8b\x4c\x24\x04\x89\x4f\x08")
    # validation result (saved EAX) = [esp+0x1c] -> [edi+0x0c]
...
def decode_connect_validate_ring(path: Path) -> dict[str, object]:
    data = path.read_bytes()
    if len(data) < 8:
        raise ValueError("connect-validation ring is too small")
    ...
    while offset + RECORD_BYTES <= len(data) and index < RECORD_CAPACITY:
        ...
        if chunk[:4] != MAGIC:
            ...
            call_index, this_ptr, valid, handle = struct.unpack_from("<IIII", chunk, 4)
            records.append(
                {
                    "index": index,
                    "callIndex": call_index,
                    "thisHex": f"0x{this_ptr:08x}",
                    "validResult": valid & 0xff,
                    "validated": (valid & 0xff) != 0,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch/decode the LOGH VII connect socket-validation ring probe.")
    ...
    if args.command == "patch":
        patch_result = apply_connect_validate_probe_patch(args.source, args.out, args.manifest_out)
        print(json.dumps(patch_result.to_json(), ensure_ascii=False, indent=2))
        return 0
    decoded = decode_connect_validate_ring(args.ring)
